...
test "broad deterministic f64 corpus is round-trip stable and byte locked" {
    ...
    for (0..sample_count) |_| {
        ...
        if (!std.math.isFinite(number)) {
            ...
            try std.testing.expectError(
                error.NonFiniteNumber,
                canonicalJsonAlloc(std.testing.allocator, .{ .float = number }),
            );
        ...
        finite_count += 1;
        const encoded = try canonicalJsonAlloc(std.testing.allocator, .{ .float = number });
        defer std.testing.allocator.free(encoded);
        ...
        const repeated = try canonicalJsonAlloc(std.testing.allocator, .{ .float = number });
        defer std.testing.allocator.free(repeated);
