...
pub fn compile(allocator: std.mem.Allocator, bytes: []const u8) !CompileResult {
    ...
    if (outer.get("execution_policy_graph")) |wrapped| {
        ...
        if (wrapped != .object) {
            return .{ .report = try singleError(allocator, .schema_invalid, "$.execution_policy_graph") };
        }
...
fn validateRich(allocator: std.mem.Allocator, root: std.json.ObjectMap) !errors.ValidationReport {
    ...
    if (!stringEquals(root, "policy_version", "EPG-v1")) try builder.add(.schema_invalid, "$.policy_version");
    ...
    if (root.get("gate") != null) try builder.add(.self_certification_forbidden, "$.gate");
    if (root.get("handoff") != null) try builder.add(.self_certification_forbidden, "$.handoff");
    try validateSourceShape(&builder, root);
    try validateHorizonShape(&builder, root);
    try validateChallengeShape(&builder, root);
    try validateInvalidatorsShape(&builder, root, temp);
    try validateRevisionShape(&builder, root, temp);

    const goal = objectField(root, "goal") orelse {
        try builder.add(.schema_invalid, "$.goal");
        return builder.finish();
    };
    const obligation_ids = try collectIds(temp, &builder, arrayField(goal, "obligations"), "obligation_id", "$.goal.obligations");
    const fact_ids = if (objectField(root, "belief")) |belief|
        try collectIds(temp, &builder, arrayField(belief, "facts"), "fact_id", "$.belief.facts")
    else blk: {
        try builder.add(.schema_invalid, "$.belief");
        break :blk &.{};
    };
    const unknown_ids = if (objectField(root, "belief")) |belief|
        try collectIds(temp, &builder, arrayField(belief, "unknowns"), "unknown_id", "$.belief.unknowns")
    else
        &.{};
    const observation_ids = try collectIds(temp, &builder, arrayField(root, "observations"), "observation_id", "$.observations");
    const action_ids = try collectIds(temp, &builder, arrayField(root, "actions"), "action_id", "$.actions");
    try validateRegimeShape(&builder, root, observation_ids);
    ...
    if (seams.len == 0) try builder.add(.architectonic_incomplete, "$.architectonic.seams");
    for (seams, 0..) |value, index| {
        const path = try std.fmt.allocPrint(temp, "$.architectonic.seams[{d}]", .{index});
        if (value != .object) {
        ...
        const id = stringField(seam, "seam_id") orelse {
            try builder.add(.schema_invalid, try suffix(temp, path, ".seam_id"));
            continue;
        };
        atom.validateStableId(id) catch try builder.add(.atom_invalid, try suffix(temp, path, ".seam_id"));
        if (contains(seam_ids.items, id)) try builder.add(.id_duplicate, try suffix(temp, path, ".seam_id")) else try seam_ids.append(temp, id);
        if (!oneOf(stringField(seam, "authority"), &.{ "source_fixed", "source_bounded", "plan_local" })) try builder.add(.schema_invalid, try suffix(temp, path, ".authority"));
        if (!oneOf(stringField(seam, "axis"), &.{ "data_shape", "behavior", "syntax_semantics", "composition", "representation", "ownership", "context", "transport", "proof" })) try builder.add(.schema_invalid, try suffix(temp, path, ".axis"));
        if (!oneOf(stringField(seam, "typed_hole"), &.{ "object", "map", "representation", "interpreter", "composition", "equivalence", "owner", "proof" })) try builder.add(.schema_invalid, try suffix(temp, path, ".typed_hole"));
        try requireStringArray(&builder, seam, "live_obligation_refs", try suffix(temp, path, ".live_obligation_refs"));
        try validateStringRefs(&builder, stringValues(seam, "live_obligation_refs"), obligation_ids, try suffix(temp, path, ".live_obligation_refs"));
        try requireStringArray(&builder, seam, "required_observation_refs", try suffix(temp, path, ".required_observation_refs"));
        try validateStringRefs(&builder, stringValues(seam, "required_observation_refs"), observation_ids, try suffix(temp, path, ".required_observation_refs"));
        try requireStringArray(&builder, seam, "compatibility_and_migration", try suffix(temp, path, ".compatibility_and_migration"));
        try requireStringArray(&builder, seam, "host_capabilities", try suffix(temp, path, ".host_capabilities"));
        try requireStringArray(&builder, seam, "decision_observation_refs", try suffix(temp, path, ".decision_observation_refs"));
        try validateStringRefs(&builder, stringValues(seam, "decision_observation_refs"), observation_ids, try suffix(temp, path, ".decision_observation_refs"));
        try requireStringArray(&builder, seam, "residual_obligations", try suffix(temp, path, ".residual_obligations"));
        try requireStringArray(&builder, seam, "invalidators", try suffix(temp, path, ".invalidators"));
        const incumbent = objectField(seam, "incumbent");
        if (incumbent == null or emptyStringField(incumbent.?, "organization")) {
            try builder.add(.architectonic_incomplete, try suffix(temp, path, ".incumbent"));
        } else {
            try requireStringArray(&builder, incumbent.?, "factor_refs", try suffix(temp, path, ".incumbent.factor_refs"));
        }
        ...
        {
            try builder.add(.architectonic_incomplete, try suffix(temp, path, ".candidate_movements"));
        }
        const disposition = stringField(seam, "disposition");
        if (!oneOf(disposition, &.{ "selected", "evidence_conditioned" })) try builder.add(.architectonic_incomplete, try suffix(temp, path, ".disposition"));
        if ((disposition == null or std.mem.eql(u8, disposition.?, "selected") or std.mem.eql(u8, disposition.?, "evidence_conditioned")) and
            emptyStringField(seam, "selected_organization")) try builder.add(.architectonic_incomplete, try suffix(temp, path, ".selected_organization"));
        if (disposition != null and std.mem.eql(u8, disposition.?, "evidence_conditioned") and
            emptyArrayField(seam, "decision_observation_refs")) try builder.add(.architectonic_incomplete, try suffix(temp, path, ".decision_observation_refs"));
        if (emptyStringField(seam, "law")) try builder.add(.architectonic_incomplete, try suffix(temp, path, ".law"));
        if (emptyStringField(seam, "falsifier")) try builder.add(.architectonic_incomplete, try suffix(temp, path, ".falsifier"));
        const boundary = objectField(seam, "boundary");
        if (boundary == null or emptyStringField(boundary.?, "owner") or emptyStringField(boundary.?, "source") or emptyStringField(boundary.?, "target")) {
            try builder.add(.architectonic_incomplete, try suffix(temp, path, ".boundary"));
        }
        const seam_factors = arrayField(seam, "factors") orelse {
            try builder.add(.architectonic_incomplete, try suffix(temp, path, ".factors"));
            continue;
        };
        if (seam_factors.len == 0) try builder.add(.architectonic_incomplete, try suffix(temp, path, ".factors"));
        for (seam_factors, 0..) |factor_value, factor_index| {
            const factor_path = try std.fmt.allocPrint(temp, "{s}.factors[{d}]", .{ path, factor_index });
            if (factor_value != .object) {
                try builder.add(.schema_invalid, factor_path);
                continue;
            }
            const factor = factor_value.object;
            const factor_id = stringField(factor, "factor_id") orelse {
                try builder.add(.schema_invalid, try suffix(temp, factor_path, ".factor_id"));
                continue;
            };
            atom.validateStableId(factor_id) catch try builder.add(.atom_invalid, try suffix(temp, factor_path, ".factor_id"));
            if (factorIndex(factors.items, factor_id) != null) {
                try builder.add(.id_duplicate, try suffix(temp, factor_path, ".factor_id"));
                continue;
            }
            if (emptyStringField(factor, "owner")) try builder.add(.architectonic_incomplete, try suffix(temp, factor_path, ".owner"));
            if (!oneOf(stringField(factor, "obligation_status"), &.{ "live", "moved", "expired", "duplicated", "invalid", "unknown" })) {
                try builder.add(.schema_invalid, try suffix(temp, factor_path, ".obligation_status"));
            }
            const factor_disposition = stringField(factor, "disposition") orelse "";
            if (!oneOf(factor_disposition, &.{ "preserve", "factor", "quotient", "ablate", "normalize", "introduce" })) {
                try builder.add(.schema_invalid, try suffix(temp, factor_path, ".disposition"));
            }
            const live_refs = stringValues(factor, "live_obligation_refs");
            if (factorRetained(factor_disposition) and live_refs.len == 0) {
                try builder.add(.factor_unearned, try suffix(temp, factor_path, ".live_obligation_refs"));
            }
            for (live_refs) |ref_value| {
                const ref = valueString(ref_value) orelse continue;
                if (!contains(obligation_ids, ref)) try builder.add(.reference_unknown, try suffix(temp, factor_path, ".live_obligation_refs"));
            }
            try factors.append(temp, .{ .id = factor_id, .disposition = factor_disposition });
        }
    }
    for (seams, 0..) |value, index| {
        if (value != .object) continue;
        const incumbent = objectField(value.object, "incumbent") orelse continue;
        const path = try std.fmt.allocPrint(temp, "$.architectonic.seams[{d}].incumbent.factor_refs", .{index});
        for (stringValues(incumbent, "factor_refs")) |ref_value| {
            const ref = valueString(ref_value) orelse continue;
            if (factorIndex(factors.items, ref) == null) try builder.add(.reference_unknown, path);
        }
    }
    try validateArchitectonicComposition(&builder, architectonic, seam_ids.items, temp);
    try validateConceptualCompression(&builder, architectonic, obligation_ids, factors.items, temp);

    const actions = arrayField(root, "actions") orelse &.{};
    var seam_bound = try temp.alloc(bool, seam_ids.items.len);
    ...
    for (actions, 0..) |value, index| {
        ...
        const path = try std.fmt.allocPrint(temp, "$.actions[{d}]", .{index});
        const action_id = stringField(action, "action_id") orelse continue;
        const seam_refs = stringValues(action, "architectonic_seam_refs");
        if (seam_refs.len == 0) try builder.add(.architectonic_unbound, try suffix(temp, path, ".architectonic_seam_refs"));
        for (seam_refs) |ref_value| {
            const ref = valueString(ref_value) orelse continue;
            if (indexOf(seam_ids.items, ref)) |seam_index| seam_bound[seam_index] = true else try builder.add(.reference_unknown, try suffix(temp, path, ".architectonic_seam_refs"));
        }
        for (stringValues(action, "realizes_factor_refs")) |ref_value| {
            const ref = valueString(ref_value) orelse continue;
            if (factorIndex(factors.items, ref)) |factor_index| {
                if (!factorRetained(factors.items[factor_index].disposition)) try builder.add(.factor_disposition_conflict, try suffix(temp, path, ".realizes_factor_refs"));
                factors.items[factor_index].realized = true;
            } else try builder.add(.reference_unknown, try suffix(temp, path, ".realizes_factor_refs"));
        }
        for (stringValues(action, "retires_factor_refs")) |ref_value| {
            const ref = valueString(ref_value) orelse continue;
            if (factorIndex(factors.items, ref)) |factor_index| {
                if (factorRetained(factors.items[factor_index].disposition)) try builder.add(.factor_disposition_conflict, try suffix(temp, path, ".retires_factor_refs"));
                factors.items[factor_index].retired = true;
            } else try builder.add(.reference_unknown, try suffix(temp, path, ".retires_factor_refs"));
        }
        for (stringValues(action, "preservation_observation_refs")) |ref_value| {
            const ref = valueString(ref_value) orelse continue;
            if (!contains(observation_ids, ref)) try builder.add(.reference_unknown, try suffix(temp, path, ".preservation_observation_refs"));
        }
        for (stringValues(action, "requires_actions")) |ref_value| {
            const ref = valueString(ref_value) orelse continue;
            if (!contains(action_ids, ref)) try builder.add(.reference_unknown, try suffix(temp, path, ".requires_actions"));
            if (std.mem.eql(u8, ref, action_id)) try builder.add(.action_cycle, try suffix(temp, path, ".requires_actions"));
        }
        const effects = objectField(action, "expected_effects") orelse {
            try builder.add(.schema_invalid, try suffix(temp, path, ".expected_effects"));
            continue;
        };
        try validateStringRefs(&builder, stringValues(effects, "facts_added"), fact_ids, try suffix(temp, path, ".expected_effects.facts_added"));
        try validateStringRefs(&builder, stringValues(effects, "unknowns_resolved"), unknown_ids, try suffix(temp, path, ".expected_effects.unknowns_resolved"));
        try validateStringRefs(&builder, stringValues(effects, "obligations_closed"), obligation_ids, try suffix(temp, path, ".expected_effects.obligations_closed"));
        try validateStringRefs(&builder, stringValues(action, "expected_observation_refs"), observation_ids, try suffix(temp, path, ".expected_observation_refs"));
        try validateStringRefs(&builder, stringValues(action, "failure_observation_refs"), observation_ids, try suffix(temp, path, ".failure_observation_refs"));
        if (isRiskyKind(stringField(action, "kind")) and emptyArrayField(action, "proof_obligations")) {
            try builder.add(.proof_missing, try suffix(temp, path, ".proof_obligations"));
        }
    }
    for (seam_bound) |bound| if (!bound) try builder.add(.architectonic_unbound, "$.architectonic.seams");
    for (factors.items) |factor| {
        if ((std.mem.eql(u8, factor.disposition, "factor") or std.mem.eql(u8, factor.disposition, "introduce")) and !factor.realized) {
            try builder.add(.factor_unearned, "$.architectonic.seams.factors");
        }
        if (!factorRetained(factor.disposition) and !factor.retired) try builder.add(.factor_disposition_conflict, "$.architectonic.seams.factors");
    }
    ...
    if (!stringEquals(policy, "selection", "lexicographic_utility")) try builder.add(.schema_invalid, "$.policy.selection");
    ...
    for (rules, 0..) |value, index| {
        ...
        const path = try std.fmt.allocPrint(temp, "$.policy.rules[{d}]", .{index});
        const priority = integerField(rule, "priority") orelse {
            try builder.add(.schema_invalid, try suffix(temp, path, ".priority"));
            continue;
        };
        if (containsInt(priorities.items, priority)) try builder.add(.id_duplicate, try suffix(temp, path, ".priority")) else try priorities.append(temp, priority);
        const candidates = stringValues(rule, "candidate_action_ids");
        for (candidates) |ref| {
            const action_ref = valueString(ref) orelse continue;
            if (!contains(action_ids, action_ref)) try builder.add(.reference_unknown, try suffix(temp, path, ".candidate_action_ids")) else if (!contains(selected_actions.items, action_ref)) try selected_actions.append(temp, action_ref);
        }
        const terminal = nullableStringField(rule, "terminal");
        if (candidates.len == 0 and terminal == null) try builder.add(.policy_dead_end, path);
        if (candidates.len > 0 and terminal != null) try builder.add(.schema_invalid, path);
    }
    for (action_ids) |id| if (!contains(selected_actions.items, id)) try builder.add(.action_unreachable, "$.actions");
    ...
    for (observations, 0..) |value, index| {
        ...
        for (outcomes) |outcome_value| {
            ...
            const prefix = try std.fmt.allocPrint(temp, "obs:{s}=", .{id});
            if (!std.mem.startsWith(u8, outcome_atom, prefix) or !validAtom(outcome_atom)) {
                try builder.add(.atom_invalid, try std.fmt.allocPrint(temp, "$.observations[{d}].outcomes", .{index}));
            }
        }
    }

    const unknowns = if (objectField(root, "belief")) |belief| arrayField(belief, "unknowns") orelse &.{} else &.{};
    for (unknowns) |value| {
        if (value != .object) continue;
        const unknown = value.object;
        const id = stringField(unknown, "unknown_id") orelse continue;
        if (stringEquals(unknown, "status", "open") and oneOf(stringField(unknown, "urgency"), &.{ "critical", "high" })) {
            if (emptyArrayField(unknown, "observation_refs")) try builder.add(.critical_unknown_unobservable, "$.belief.unknowns.observation_refs");
            if (!actionResolves(actions, id)) try builder.add(.critical_unknown_unobservable, "$.belief.unknowns");
        }
    }
    for (obligation_ids) |id| if (!actionCloses(actions, id)) try builder.add(.obligation_uncovered, "$.goal.obligations");
    try validateRiskShields(&builder, root, action_ids, temp);
...
fn lowerRich(allocator: std.mem.Allocator, root: std.json.ObjectMap) ![]u8 {
    ...
    defer arena.deinit();
    const temp = arena.allocator();
    const declared_atoms = try collectDeclaredAtoms(temp, root);
    ...
    try writeStringSlice(writer, declared_atoms);
    ...
    if (arrayField(root, "observations")) |observations| {
        for (observations, 0..) |value, index| {
            if (index > 0) try writer.writeByte(',');
            const observation = value.object;
            try writer.writeByte('{');
            try writeNamedString(writer, "id", stringField(observation, "observation_id").?);
            ...
            if (arrayField(observation, "outcomes")) |outcomes| {
                for (outcomes, 0..) |outcome, outcome_index| {
                    ...
                    try writeJsonString(writer, stringField(outcome.object, "outcome").?);
                }
            }
            try writer.writeAll("]}");
        }
    ...
    const actions = arrayField(root, "actions").?;
    for (actions, 0..) |value, index| {
        if (index > 0) try writer.writeByte(',');
        try writeLoweredAction(temp, writer, root, value.object);
    }
    ...
    var terminal_it = terminals.iterator();
    var first_terminal = true;
    while (terminal_it.next()) |entry| {
        if (entry.value_ptr.* != .object) continue;
        if (!first_terminal) try writer.writeByte(',');
        first_terminal = false;
        try writer.writeByte('{');
        try writeNamedString(writer, "id", entry.key_ptr.*);
        try writer.writeAll(",\"condition\":");
        try writeValue(writer, entry.value_ptr.object.get("when") orelse emptyCondition());
        try writer.writeByte('}');
    }
    ...
    for (dimensions, 0..) |value, index| {
        if (index > 0) try writer.writeByte(',');
        const dimension = value.object;
        try writer.writeByte('{');
        try writeNamedString(writer, "id", stringField(dimension, "dimension_id").?);
...
fn writeLoweredAction(
    ...
    if (objectField(root, "belief")) |belief| {
        ...
        for (stringValues(expected, "facts_added")) |fact_id_value| {
            ...
            if (atomForId(facts, "fact_id", fact_id)) |fact_atom| try appendUnique(&effects, allocator, fact_atom);
        }
    }
    for (stringValues(expected, "unknowns_resolved")) |id_value| {
        const id = valueString(id_value) orelse continue;
        try appendUnique(&effects, allocator, try std.fmt.allocPrint(allocator, "unknown:{s}=resolved", .{id}));
    }
    for (stringValues(expected, "obligations_closed")) |id_value| {
        const id = valueString(id_value) orelse continue;
        try appendUnique(&effects, allocator, try std.fmt.allocPrint(allocator, "obligation:{s}=closed", .{id}));
    }
...
fn writeLoweredShields(writer: *std.Io.Writer, root: std.json.ObjectMap) !void {
    ...
    for (rules) |value| {
        ...
        for (actions) |action_value| {
            ...
            const action_id = stringField(action, "action_id") orelse continue;
            const kind = stringField(action, "kind") orelse "";
            if (!stringArrayContains(rule, "forbids_action_ids", action_id) and !stringArrayContains(rule, "forbids_action_kinds", kind)) continue;
...
fn writeShieldRow(writer: *std.Io.Writer, action_id: []const u8, rule: std.json.ObjectMap, missing_atom: ?[]const u8) !void {
    try writer.writeByte('{');
    try writeNamedString(writer, "action_id", action_id);
    try writer.writeAll(",\"response\":");
    const source_response = stringField(rule, "response") orelse "block";
    try writeJsonString(writer, if (std.mem.eql(u8, source_response, "block")) "blocked" else source_response);
    try writer.writeAll(",\"condition\":");
...
fn lowerInitialState(allocator: std.mem.Allocator, root: std.json.ObjectMap, policy_digest: []const u8) !?schema.State {
    ...
    const current = objectField(initial, "current_potential") orelse objectField(potential, "initial").?;
...
fn collectDeclaredAtoms(allocator: std.mem.Allocator, root: std.json.ObjectMap) ![]const []const u8 {
    var result: std.ArrayList([]const u8) = .empty;
    const goal = objectField(root, "goal").?;
    if (arrayField(goal, "terminal_predicates")) |rows| {
        for (rows) |value| if (value == .object) {
            if (stringField(value.object, "atom")) |value_atom| try appendUnique(&result, allocator, value_atom);
        };
    }
    if (arrayField(goal, "safety_invariants")) |rows| {
        for (rows) |value| if (value == .object) {
            if (stringField(value.object, "violation_atom")) |value_atom| try appendUnique(&result, allocator, value_atom);
        };
    }
    ...
    if (arrayField(goal, "obligations")) |rows| {
        for (rows) |value| if (value == .object) {
            if (stringField(value.object, "obligation_id")) |id| {
                try appendUnique(&result, allocator, try std.fmt.allocPrint(allocator, "obligation:{s}=closed", .{id}));
            }
        };
    }
    if (objectField(root, "belief")) |belief| {
        ...
        if (arrayField(belief, "unknowns")) |rows| {
            for (rows) |value| if (value == .object) {
                if (stringField(value.object, "unknown_id")) |id| {
                    try appendUnique(&result, allocator, try std.fmt.allocPrint(allocator, "unknown:{s}=resolved", .{id}));
                    try appendUnique(&result, allocator, try std.fmt.allocPrint(allocator, "unknown:{s}=blocked", .{id}));
                }
            };
        }
    }
        ...
        for (rows) |value| if (value == .object) {
            if (arrayField(value.object, "outcomes")) |outcomes| {
                for (outcomes) |outcome| if (outcome == .object) {
                    if (stringField(outcome.object, "atom")) |value_atom| try appendUnique(&result, allocator, value_atom);
                };
            }
        };
    }
    if (arrayField(root, "actions")) |rows| {
        for (rows) |value| if (value == .object) {
            if (stringField(value.object, "action_id")) |id| {
                try appendUnique(&result, allocator, try std.fmt.allocPrint(allocator, "action:{s}=success", .{id}));
                try appendUnique(&result, allocator, try std.fmt.allocPrint(allocator, "action:{s}=failure", .{id}));
            }
        };
    }
...
fn validateConditions(builder: *errors.Builder, root: std.json.ObjectMap, allocator: std.mem.Allocator) !void {
    const declared = try collectDeclaredAtoms(allocator, root);
    const actions = arrayField(root, "actions") orelse &.{};
    for (actions, 0..) |value, index| {
        if (value != .object) continue;
        try validateCondition(builder, value.object.get("preconditions"), declared, try std.fmt.allocPrint(allocator, "$.actions[{d}].preconditions", .{index}));
        ...
        if (rollback) |row| for (stringValues(row, "trigger_atoms")) |item_value| {
            ...
            if (!validAtom(item)) try builder.add(.atom_invalid, "$.actions.rollback.trigger_atoms");
            if (!contains(declared, item)) try builder.add(.atom_unknown, "$.actions.rollback.trigger_atoms");
        };
    }
    if (objectField(root, "policy")) |policy| {
        const rules = arrayField(policy, "rules") orelse &.{};
        for (rules, 0..) |value, index| if (value == .object) {
            try validateCondition(builder, value.object.get("when"), declared, try std.fmt.allocPrint(allocator, "$.policy.rules[{d}].when", .{index}));
            for (stringValues(value.object, "replan_if_atoms")) |item_value| {
                const item = valueString(item_value) orelse continue;
                if (!validAtom(item)) try builder.add(.atom_invalid, "$.policy.rules.replan_if_atoms");
                if (!contains(declared, item)) try builder.add(.atom_unknown, "$.policy.rules.replan_if_atoms");
            }
        };
    }
    if (objectField(root, "terminal_states")) |terminals| {
        ...
        while (it.next()) |entry| if (entry.value_ptr.* == .object) {
            try validateCondition(builder, entry.value_ptr.object.get("when"), declared, "$.terminal_states.when");
        };
    }
    if (objectField(root, "safety_shield")) |shield| {
        const rules = arrayField(shield, "rules") orelse &.{};
        for (rules) |value| if (value == .object) {
            try validateCondition(builder, value.object.get("when"), declared, "$.safety_shield.rules.when");
            for (stringValues(value.object, "requires_atoms")) |item_value| {
                const item = valueString(item_value) orelse continue;
                if (!validAtom(item)) try builder.add(.atom_invalid, "$.safety_shield.rules.requires_atoms");
                if (!contains(declared, item)) try builder.add(.atom_unknown, "$.safety_shield.rules.requires_atoms");
            }
        };
    }
}

fn validateCondition(builder: *errors.Builder, maybe_value: ?std.json.Value, declared: []const []const u8, path: []const u8) !void {
...
fn validateRiskShields(
    ...
    const shield = objectField(root, "safety_shield") orelse {
        for (actions) |value| if (value == .object and isRiskyKind(stringField(value.object, "kind"))) {
    ...
    for (rules, 0..) |value, index| {
        ...
        if (!oneOf(stringField(row, "response"), &.{ "block", "rollback", "return_to_spec" })) {
            try builder.add(.schema_invalid, try std.fmt.allocPrint(allocator, "$.safety_shield.rules[{d}].response", .{index}));
        }
        for (stringValues(row, "forbids_action_ids")) |ref_value| {
            const ref = valueString(ref_value) orelse continue;
            if (!contains(action_ids, ref)) try builder.add(.reference_unknown, "$.safety_shield.rules.forbids_action_ids");
        }
    }
    for (actions) |value| {
        if (value != .object) continue;
...
fn validateSourceShape(builder: *errors.Builder, root: std.json.ObjectMap) !void {
    ...
    if (!oneOf(stringField(source, "mode"), &.{ "spec_handoff", "direct_brief", "existing_policy_revision" })) {
        try builder.add(.schema_invalid, "$.source.mode");
    }
    if (emptyStringField(source, "authority")) try builder.add(.schema_invalid, "$.source.authority");
    try requireStringArray(builder, source, "source_refs", "$.source.source_refs");
    if (emptyArrayField(source, "source_refs")) try builder.add(.schema_invalid, "$.source.source_refs");
    const source_digest = stringField(source, "source_digest") orelse "";
    if (!validSha256Digest(source_digest)) try builder.add(.schema_invalid, "$.source.source_digest");
    try requireStringArray(builder, source, "locked_decision_refs", "$.source.locked_decision_refs");
    ...
    if (!oneOf(repo_bound, &.{ "yes", "no", "unknown" })) try builder.add(.schema_invalid, "$.source.artifact_state.repo_bound");
    if (repo_bound != null and std.mem.eql(u8, repo_bound.?, "yes")) {
        inline for ([_][]const u8{ "repository", "branch", "base", "head", "dirty_fingerprint" }) |key| {
            if (emptyStringField(artifact_state, key)) try builder.add(.schema_invalid, "$.source.artifact_state");
        }
    }
}
...
fn validateRegimeShape(
    ...
    if (!oneOf(stringField(regime, "kind"), &.{ "clear", "complicated", "complex", "chaotic" })) try builder.add(.schema_invalid, "$.regime.kind");
    if (!oneOf(stringField(regime, "confidence"), &.{ "high", "medium", "low" })) try builder.add(.schema_invalid, "$.regime.confidence");
    if (emptyStringField(regime, "rationale")) try builder.add(.schema_invalid, "$.regime.rationale");
    try requireStringArray(builder, regime, "reclassify_on_observation_refs", "$.regime.reclassify_on_observation_refs");
...
fn validateHorizonShape(builder: *errors.Builder, root: std.json.ObjectMap) !void {
    const horizon = objectField(root, "horizon") orelse {
        try builder.add(.schema_invalid, "$.horizon");
        return;
    };
    inline for ([_][]const u8{ "mutation_actions_max", "evidence_actions_max", "delivery_transitions_max" }) |key| {
        const value = integerField(horizon, key);
        if (value == null or value.? < 0) try builder.add(.schema_invalid, "$.horizon");
    }
}

fn validateChallengeShape(builder: *errors.Builder, root: std.json.ObjectMap) !void {
    const challenge = objectField(root, "challenge") orelse {
        try builder.add(.schema_invalid, "$.challenge");
        return;
    };
    if (emptyStringField(challenge, "candidate")) try builder.add(.schema_invalid, "$.challenge.candidate");
    if (!oneOf(stringField(challenge, "disposition"), &.{ "adopt", "reject", "defer", "return_to_spec", "none" })) {
        try builder.add(.schema_invalid, "$.challenge.disposition");
    }
    if (emptyStringField(challenge, "reason")) try builder.add(.schema_invalid, "$.challenge.reason");
    try requireStringArray(builder, challenge, "affected_refs", "$.challenge.affected_refs");
    if (boolField(challenge, "source_change_required") == null) try builder.add(.schema_invalid, "$.challenge.source_change_required");
}
...
fn validateInvalidatorsShape(
    ...
    for (invalidators.array.items, 0..) |value, index| {
        ...
        atom.validateStableId(id) catch try builder.add(.atom_invalid, try suffix(allocator, path, ".invalidator_id"));
        if (contains(ids.items, id)) try builder.add(.id_duplicate, try suffix(allocator, path, ".invalidator_id")) else try ids.append(allocator, id);
        if (emptyStringField(value.object, "condition")) try builder.add(.schema_invalid, try suffix(allocator, path, ".condition"));
        if (emptyStringField(value.object, "required_action")) try builder.add(.schema_invalid, try suffix(allocator, path, ".required_action"));
        try requireStringArray(builder, value.object, "affected_refs", try suffix(allocator, path, ".affected_refs"));
    }
}
...
fn validateRevisionShape(
    ...
    if (architectonic_changes != .array) try builder.add(.schema_invalid, "$.revision_summary.architectonic_changes");
    ...
    inline for ([_][]const u8{
        ...
        const path = try std.fmt.allocPrint(allocator, "$.revision_summary.plan_transport.{s}", .{key});
    ...
    if (squares != .array) try builder.add(.schema_invalid, "$.revision_summary.square_results");
}
...
fn validateArchitectonicComposition(
    ...
    const edges_value = composition.get("seam_dependency_edges") orelse {
        try builder.add(.schema_invalid, "$.architectonic.composition.seam_dependency_edges");
        return;
    };
    if (edges_value != .array) {
        try builder.add(.schema_invalid, "$.architectonic.composition.seam_dependency_edges");
    } else for (edges_value.array.items, 0..) |value, index| {
        const path = try std.fmt.allocPrint(allocator, "$.architectonic.composition.seam_dependency_edges[{d}]", .{index});
        if (value != .object) {
            try builder.add(.schema_invalid, path);
            continue;
        }
        ...
        if (from == null or !contains(seam_ids, from.?)) try builder.add(.reference_unknown, try suffix(allocator, path, ".from_seam_ref"));
        if (to == null or !contains(seam_ids, to.?)) try builder.add(.reference_unknown, try suffix(allocator, path, ".to_seam_ref"));
        if (emptyStringField(value.object, "relation")) try builder.add(.schema_invalid, try suffix(allocator, path, ".relation"));
    }

    const sets_value = composition.get("independent_seam_sets") orelse {
        try builder.add(.schema_invalid, "$.architectonic.composition.independent_seam_sets");
        return;
    };
    if (sets_value != .array) {
        try builder.add(.schema_invalid, "$.architectonic.composition.independent_seam_sets");
    } else for (sets_value.array.items, 0..) |set_value, set_index| {
        const path = try std.fmt.allocPrint(allocator, "$.architectonic.composition.independent_seam_sets[{d}]", .{set_index});
        if (set_value != .array or set_value.array.items.len == 0) {
            try builder.add(.schema_invalid, path);
            continue;
        }
        for (set_value.array.items) |ref_value| {
            if (ref_value != .string or !contains(seam_ids, if (ref_value == .string) ref_value.string else "")) {
...
fn validateConceptualCompression(
    ...
    inline for ([_][]const u8{
        ...
        const path = try std.fmt.allocPrint(allocator, "$.architectonic.conceptual_compression.{s}", .{key});
    ...
    for (stringValues(compression, "independent_factor_refs")) |ref_value| {
        const ref = valueString(ref_value) orelse continue;
        const index = factorIndex(factors, ref) orelse {
            try builder.add(.reference_unknown, "$.architectonic.conceptual_compression.independent_factor_refs");
            continue;
        };
        if (!factorRetained(factors[index].disposition)) {
            try builder.add(.factor_disposition_conflict, "$.architectonic.conceptual_compression.independent_factor_refs");
        }
    }
    for (stringValues(compression, "dominated_factor_refs")) |ref_value| {
        const ref = valueString(ref_value) orelse continue;
        const index = factorIndex(factors, ref) orelse {
            try builder.add(.reference_unknown, "$.architectonic.conceptual_compression.dominated_factor_refs");
            continue;
        };
        if (factorRetained(factors[index].disposition)) {
            try builder.add(.factor_disposition_conflict, "$.architectonic.conceptual_compression.dominated_factor_refs");
        }
    }
}

fn validateLoweringShape(builder: *errors.Builder, root: std.json.ObjectMap, allocator: std.mem.Allocator) !void {
    ...
    for (observations.array.items, 0..) |value, index| {
        ...
        for (outcomes.array.items) |outcome| {
            if (outcome != .object or emptyStringField(outcome.object, "outcome") or emptyStringField(outcome.object, "atom")) {
                try builder.add(.schema_invalid, try std.fmt.allocPrint(allocator, "$.observations[{d}].outcomes", .{index}));
            }
        }
    }
    ...
    if (utility_order != .array or utility_order.array.items.len == 0) {
        try builder.add(.schema_invalid, "$.policy.utility_order");
    } else for (utility_order.array.items, 0..) |value, index| {
        ...
        const entry = it.next().?;
        if (!std.mem.eql(u8, entry.key_ptr.*, "maximize") and !std.mem.eql(u8, entry.key_ptr.*, "minimize")) {
            try builder.add(.schema_invalid, "$.policy.utility_order");
        }
        ...
        if (contains(utility_keys.items, entry.value_ptr.string)) {
            try builder.add(.id_duplicate, try std.fmt.allocPrint(allocator, "$.policy.utility_order[{d}]", .{index}));
        } else try utility_keys.append(allocator, entry.value_ptr.string);
    }
    ...
    for (actions, 0..) |value, index| {
        ...
        try requireStringArray(builder, action, "requires_actions", try suffix(allocator, path, ".requires_actions"));
        try requireStringArray(builder, action, "architectonic_seam_refs", try suffix(allocator, path, ".architectonic_seam_refs"));
        try requireStringArray(builder, action, "realizes_factor_refs", try suffix(allocator, path, ".realizes_factor_refs"));
        try requireStringArray(builder, action, "retires_factor_refs", try suffix(allocator, path, ".retires_factor_refs"));
        try requireStringArray(builder, action, "preservation_observation_refs", try suffix(allocator, path, ".preservation_observation_refs"));
        try requireStringArray(builder, action, "expected_observation_refs", try suffix(allocator, path, ".expected_observation_refs"));
        try requireStringArray(builder, action, "failure_observation_refs", try suffix(allocator, path, ".failure_observation_refs"));
        if (boolField(action, "repeatable") == null) try builder.add(.schema_invalid, try suffix(allocator, path, ".repeatable"));
        const effects = objectField(action, "expected_effects") orelse {
            try builder.add(.schema_invalid, try suffix(allocator, path, ".expected_effects"));
            continue;
        };
        try requireStringArray(builder, effects, "facts_added", try suffix(allocator, path, ".expected_effects.facts_added"));
        try requireStringArray(builder, effects, "unknowns_resolved", try suffix(allocator, path, ".expected_effects.unknowns_resolved"));
        try requireStringArray(builder, effects, "obligations_closed", try suffix(allocator, path, ".expected_effects.obligations_closed"));
        const utility = objectField(action, "utility") orelse {
            try builder.add(.schema_invalid, try suffix(allocator, path, ".utility"));
            continue;
        };
        for (utility_keys.items) |key| {
    ...
    if (rules_value != .array) {
        ...
        if (emptyStringField(value.object, "rule_id")) try builder.add(.schema_invalid, try suffix(allocator, path, ".rule_id"));
        try requireStringArray(builder, value.object, "candidate_action_ids", try suffix(allocator, path, ".candidate_action_ids"));
        const terminal = value.object.get("terminal") orelse {
            try builder.add(.schema_invalid, try suffix(allocator, path, ".terminal"));
            continue;
        };
        if (terminal != .null and terminal != .string) try builder.add(.schema_invalid, try suffix(allocator, path, ".terminal"));
    }
    ...
    try requireStringArray(builder, potential, "lexicographic_order", "$.potential.lexicographic_order");
    ...
    if (objectField(potential, "initial") == null) try builder.add(.schema_invalid, "$.potential.initial");

    const initial = objectField(root, "initial_state") orelse {
        try builder.add(.schema_invalid, "$.initial_state");
        return;
    };
    if (emptyStringField(initial, "state_id")) try builder.add(.schema_invalid, "$.initial_state.state_id");
    try requireStringArray(builder, initial, "satisfied_atoms", "$.initial_state.satisfied_atoms");
    try requireStringArray(builder, initial, "completed_actions", "$.initial_state.completed_actions");
    try requireStringArray(builder, initial, "failed_actions", "$.initial_state.failed_actions");
    if (objectField(initial, "current_potential") == null) try builder.add(.schema_invalid, "$.initial_state.current_potential");
}

fn requireStringArray(builder: *errors.Builder, object: std.json.ObjectMap, key: []const u8, path: []const u8) !void {
    ...
    for (value.array.items) |item| if (item != .string) try builder.add(.schema_invalid, path);
}
...
fn collectIds(
    ...
    for (rows, 0..) |value, index| {
        ...
        if (contains(ids.items, id)) {
            try builder.add(.id_duplicate, try std.fmt.allocPrint(allocator, "{s}[{d}].{s}", .{ path, index, key }));
...
fn atomForId(rows: []const std.json.Value, id_key: []const u8, id: []const u8) ?[]const u8 {
    for (rows) |value| {
        if (value != .object) continue;
        if (std.mem.eql(u8, stringField(value.object, id_key) orelse "", id)) return stringField(value.object, "atom");
    }
    return null;
}

fn requireString(builder: *errors.Builder, root: std.json.ObjectMap, key: []const u8, path: []const u8) !void {
    if (emptyStringField(root, key)) try builder.add(.schema_invalid, path);
}

fn requireInteger(builder: *errors.Builder, root: std.json.ObjectMap, key: []const u8, path: []const u8) !void {
    if (integerField(root, key) == null) try builder.add(.schema_invalid, path);
}

fn singleError(allocator: std.mem.Allocator, code: errors.ErrorCode, path: []const u8) !errors.ValidationReport {
    var builder = errors.Builder.init(allocator);
    defer builder.deinit();
