...
const ReceiptValidator = struct {
    ...
    fn validate(self: *ReceiptValidator, policy: *const schema.Policy, state: *const schema.State, decision: *const schema.Decision, receipt: *const schema.TransitionReceipt) !void {
        ...
        try self.expectStringEqual(receipt_root, "policy_digest", stringField(state_root, "policy_digest"), .receipt_identity_mismatch, "$.policy_digest");
    ...
    fn validatePrediction(self: *ReceiptValidator, action: std.json.ObjectMap, receipt: std.json.ObjectMap, result: []const u8) !void {
        ...
        if (!sameStringMultiset(predicted, contract)) try self.add(.receipt_prediction_mismatch, "$.predicted_effects");
    }
    ...
    fn validateProof(self: *ReceiptValidator, action: std.json.ObjectMap, receipt: std.json.ObjectMap, result: []const u8) !void {
        ...
        if (proof_refs != .array or proof_refs.array.items.len == 0) try self.add(.proof_missing, "$.proof_refs");
    }
    ...
    fn validatePotential(self: *ReceiptValidator, receipt: std.json.ObjectMap) !void {
        ...
        const observed_potential = observed.get("potential") orelse return;
...
fn sameStringMultiset(a: []const std.json.Value, b: []const std.json.Value) bool {
    if (a.len != b.len) return false;
    var used: [64]bool = undefined;
    if (b.len > used.len) return false;
    @memset(used[0..b.len], false);
    for (a) |item| {
...
test "valid receipt validates and applies monotone state atoms" {
    var policy = try schema.parseArtifact(schema.Policy, std.testing.allocator,
        \\{"policy_id":"p","revision":1,"actions":[{"id":"a","results":{"success":["fact:done"]}}],"observations":[{"id":"obs1","outcomes":["ok"]}]}
    );
    defer policy.deinit(std.testing.allocator);
    var state = try schema.parseArtifact(schema.State, std.testing.allocator,
        \\{"policy_id":"p","revision":1,"policy_digest":"sha256:p","state_id":"s1","state_digest":"sha256:s1","satisfied_atoms":["fact:start"],"potential":[1]}
    );
    ...
    var receipt = try schema.parseArtifact(schema.TransitionReceipt, std.testing.allocator,
        \\{"receipt_id":"t1","policy_id":"p","revision":1,"policy_digest":"sha256:p","decision_id":"d1","action_id":"a","result":"success","predicted_effects":["fact:done"],"observed":{"facts":["fact:done"],"observations":{"obs1":"ok"},"resolved_unknowns":["u"],"closed_obligations":["o"],"potential":[0]},"state_after":{"state_id":"s2","state_digest":"sha256:s2","potential":[0]}}
    );
