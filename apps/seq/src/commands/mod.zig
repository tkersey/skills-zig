...
fn getEnvVarOwned(allocator: std.mem.Allocator, comptime key: [:0]const u8) ![]u8 {
    if (@import("builtin").is_test and std.mem.eql(u8, key, "CODEX_THREAD_ID")) {
        if (test_codex_thread_id) |value| return allocator.dupe(u8, value);
    }
...
fn printCommandHelp(cmd: lib.Command) !void {
    ...
    const body = switch (cmd) {
        ...
        ,
        .skill_success_rank =>
        \\usage: seq skill-success-rank [--skill <name>] [--mode summary|sessions] [--last <Nm|Nh|Nd>|--since <iso>] [--until <iso>] [--exclude-current] [--limit N] [--format table|json|csv|jsonl]
        ,
        .skill_trend =>
        ...
        ,
        .skill_audit =>
        \\usage: seq skill-audit [--skill <name>] [--mode summary|mentions|trend|activation] [--roles <csv>] [--last <Nm|Nh|Nd>|--since <iso>] [--until <iso>] [--exclude-current] [--limit N] [--format table|json|csv|jsonl]
        \\extra options:
...
fn isValidSkillAuditMode(text: []const u8) bool {
    return std.mem.eql(u8, text, "summary") or
        ...
        std.mem.eql(u8, text, "trend") or
        std.mem.eql(u8, text, "activation");
}
...
fn collectWorkflowAuditRows(
    ...
    var where: std.ArrayList(spec.WhereClause) = .empty;
    defer where.deinit(allocator);
    const last_bounds = try QueryLiftCommands.appendLastWindowBoundsForField(allocator, &where, "timestamp", opts);
...
fn cmdResolveChurnAudit(allocator: std.mem.Allocator, sessions_root: []const u8, opts: Options) !void {
    ...
    for (paths.items) |path| {
        ...
        var signals = summarizeResolveSession(messages, parsed, opts);
        ...
        try recordResolveTools(allocator, &audit, parsed, repo_root, opts, &signals);

        if (signals.mutation_session) {
            audit.permits.required += 1;
            if (!signals.permit_emitted) {
                audit.permits.missing += 1;
                audit.compliance.mutations_without_permit += 1;
            }
        }
...
fn resolveSessionOverlapsWindow(start_opt: ?[]const u8, end_opt: ?[]const u8, opts: Options) bool {
    const start_ms = if (start_opt) |value| time_utils.parseIsoTimestampMillis(value) else null;
    const end_ms = if (end_opt) |value| time_utils.parseIsoTimestampMillis(value) else start_ms;
    const effective_start = start_ms orelse end_ms orelse return false;
    const effective_end = end_ms orelse effective_start;
    if (opts.since) |since| {
        const since_ms = time_utils.parseIsoTimestampMillis(since) orelse return false;
        if (effective_end < since_ms) return false;
    }
    if (opts.until) |until| {
        const until_ms = time_utils.parseIsoTimestampMillis(until) orelse return false;
...
fn toolHasCompletedResolveReviewEvidence(tool: canonical_trace.ToolLifecycleRecord) bool {
    ...
    return commandContainsReviewInvocation(cmd);
}

fn commandContainsReviewInvocation(cmd: []const u8) bool {
    var previous: ?[]const u8 = null;
    var tokens = std.mem.tokenizeAny(u8, cmd, " \t\r\n;&|()");
    while (tokens.next()) |raw| {
        const token = std.mem.trim(u8, raw, "\"'`.,{}[]");
        if (token.len == 0) continue;
        if (std.ascii.eqlIgnoreCase(token, "cas_review_session")) return true;
        if (previous) |prev| {
            if (std.ascii.eqlIgnoreCase(prev, "cas") and std.ascii.eqlIgnoreCase(token, "review_session")) return true;
            if (std.ascii.eqlIgnoreCase(prev, "codex") and std.ascii.eqlIgnoreCase(token, "review")) return true;
        }
        previous = token;
    }
    return false;
...
fn recordResolveMessages(
    ...
    for (messages) |message| {
        ...
        if (containsAnyIgnoreCaseAscii(text, &.{ "normal_form_status: proposed", "normal form status: proposed", "status: proposed" })) audit.normal_forms.proposed += 1;
        ...
        if (containsAnyIgnoreCaseAscii(text, &.{ "fuse_state: tripped", "delivery_mutation_frozen: yes" })) {
            ...
            if (message.timestamp) |ts| {
                if (time_utils.parseIsoTimestampMillis(ts)) |ms| {
                    if (signals.fuse_tripped_at_ms == null or ms < signals.fuse_tripped_at_ms.?) signals.fuse_tripped_at_ms = ms;
                }
            }
        }
...
fn recordResolveTools(
    ...
    for (parsed.tools.items) |tool| {
        ...
        if (toolHasCompletedResolveReviewEvidence(tool)) {
            const head = extractReviewHeadFromTool(tool) orelse "unknown";
        ...
        if (tool.kind == .patch_apply and tool.lifecycle_status == .completed and tool.patch_success != false) {
            ...
            if (toolOccursAfterFuse(parsed, tool, signals)) {
...
fn toolTimestampSatisfiesBounds(parsed: canonical_trace.CanonicalSessionTrace, tool: canonical_trace.ToolLifecycleRecord, opts: Options) bool {
    const timestamp = toolTimestamp(parsed, tool) orelse return false;
    return timestampSatisfiesBounds(timestamp, opts);
}

fn toolTimestampMillis(parsed: canonical_trace.CanonicalSessionTrace, tool: canonical_trace.ToolLifecycleRecord) ?i64 {
    const timestamp = toolTimestamp(parsed, tool) orelse return null;
    return time_utils.parseIsoTimestampMillis(timestamp);
}

fn toolTimestamp(parsed: canonical_trace.CanonicalSessionTrace, tool: canonical_trace.ToolLifecycleRecord) ?[]const u8 {
    if (tool.completed_at) |value| return value;
    if (tool.started_at) |value| return value;
    if (tool.turn_index) |tool_turn_index| {
        for (parsed.turns.items) |turn| {
            if (turn.turn_index != tool_turn_index) continue;
            if (turn.completed_at) |value| return value;
            if (turn.started_at) |value| return value;
            break;
        }
    }
    return null;
...
fn toolOccursAfterFuse(parsed: canonical_trace.CanonicalSessionTrace, tool: canonical_trace.ToolLifecycleRecord, signals: *const ResolveSessionSignals) bool {
    const fuse_ms = signals.fuse_tripped_at_ms orelse return false;
    const tool_ms = toolTimestampMillis(parsed, tool) orelse return false;
    return tool_ms > fuse_ms;
}

fn toolIsInRepoScope(allocator: std.mem.Allocator, repo_root: []const u8, session_cwd: ?[]const u8, tool: canonical_trace.ToolLifecycleRecord) bool {
    const cwd = tool.cwd orelse session_cwd orelse return false;
...
fn extractReviewHeadFromTool(tool: canonical_trace.ToolLifecycleRecord) ?[]const u8 {
    if (tool.output_text) |text| {
        if (extractFlatJsonStringField(text, "headSha")) |head| return head;
        if (extractFlatJsonStringField(text, "head_sha")) |head| return head;
        if (extractReviewHead(text)) |head| return head;
    }
    if (tool.command_text) |text| {
        if (extractReviewHead(text)) |head| return head;
    }
    if (tool.input_text) |text| {
        if (extractReviewHead(text)) |head| return head;
    }
    if (tool.arguments_json) |text| {
        if (extractReviewHead(text)) |head| return head;
    }
    return null;
}
...
fn extractReviewHead(text: []const u8) ?[]const u8 {
    ...
    while (lines.next()) |line_raw| {
        const line = std.mem.trim(u8, line_raw, " \t\r\n,");
        if (line.len == 0) continue;
        if (std.mem.indexOf(u8, line, "headSha")) |idx| return tokenAfterMarker(line[idx..], ':');
...
fn tokenAfterMarker(text: []const u8, marker: u8) ?[]const u8 {
    ...
    var start = start_raw;
    while (start < text.len and (std.ascii.isWhitespace(text[start]) or text[start] == '"' or text[start] == '\'')) : (start += 1) {}
    var end = start;
...
fn classifyResolvePath(path_raw: []const u8) ResolvePathKind {
    ...
    if (containsAnyIgnoreCaseAscii(path, &.{ "/test", "tests/", "test/", "testdata", "fixtures", "__tests__", "__fixtures__", "spec/" })) return .tests;
    ...
    if (std.mem.endsWith(u8, path, ".test.ts") or
        std.mem.endsWith(u8, path, ".test.tsx") or
        std.mem.endsWith(u8, path, ".test.js") or
        std.mem.endsWith(u8, path, ".test.jsx") or
        std.mem.endsWith(u8, path, ".spec.ts") or
        std.mem.endsWith(u8, path, ".spec.tsx") or
        std.mem.endsWith(u8, path, ".spec.js") or
        std.mem.endsWith(u8, path, ".spec.jsx"))
        return .tests;
    if (containsAnyIgnoreCaseAscii(path, &.{ "src/", "cmd/", "lib/", "libs/", "app/", "apps/", "pkg/", "internal/", "crates/", "packages/", "tools/", "bin/" })) return .production;
    if (isCodeLikeProductionPath(path)) return .production;
    if (std.mem.eql(u8, path, "build.zig") or std.mem.endsWith(u8, path, "/build.zig")) return .production;
    return .other;
}

fn isCodeLikeProductionPath(path: []const u8) bool {
    return std.mem.endsWith(u8, path, ".zig") or
        ...
        std.mem.endsWith(u8, path, ".java") or
        std.mem.endsWith(u8, path, ".kt") or
        std.mem.endsWith(u8, path, ".swift") or
        std.mem.endsWith(u8, path, ".rb") or
        std.mem.endsWith(u8, path, ".sh") or
        std.mem.endsWith(u8, path, ".sql") or
        std.mem.endsWith(u8, path, ".lua");
}
...
const QueryLiftCommands = struct {
    ...
    const skill_audit_activation_columns = [_][]const u8{ "skill", "evidence_kind", "rows", "sessions", "first_seen", "last_seen", "example", "activation_verdict" };

    const SkillActivationKind = enum {
        explicit_user_call,
    ...
    const SkillActivationState = struct {
        ...
        fn init(allocator: std.mem.Allocator, kind: SkillActivationKind) SkillActivationState {
            return .{
                .kind = kind,
                .sessions = StringSet.init(allocator),
            };
        ...
        fn deinit(self: *SkillActivationState, allocator: std.mem.Allocator) void {
            self.sessions.deinit();
            if (self.first_seen) |value| allocator.free(value);
            if (self.last_seen) |value| allocator.free(value);
            if (self.example) |value| allocator.free(value);
        }

        fn record(
            self: *SkillActivationState,
            allocator: std.mem.Allocator,
            path: []const u8,
            timestamp: ?[]const u8,
            snippet: []const u8,
            ...
            self.rows += 1;
            try self.sessions.put(path);
            if (timestamp) |ts| {
                if (self.first_seen == null or compareNormalizedTimestamp(ts, self.first_seen.?) == .lt) {
                    if (self.first_seen) |value| allocator.free(value);
                    self.first_seen = try allocator.dupe(u8, ts);
                }
                if (self.last_seen == null or compareNormalizedTimestamp(ts, self.last_seen.?) == .gt) {
                    if (self.last_seen) |value| allocator.free(value);
                    self.last_seen = try allocator.dupe(u8, ts);
                }
            }
            if (self.example == null) {
                self.example = try activationSnippet(allocator, snippet, 160);
            }
        }
    };

    const SkillActivationWindow = struct {
        since_ms: ?i64 = null,
        until_ms: ?i64 = null,
    };

    fn activationKindName(kind: SkillActivationKind) []const u8 {
        return switch (kind) {
            .explicit_user_call => "explicit_user_call",
            .implicit_assistant_call => "implicit_assistant_call",
    ...
    fn activationVerdict(kind: SkillActivationKind) []const u8 {
        return switch (kind) {
            .explicit_user_call, .implicit_assistant_call => "activation",
            .injected_skill_block, .skill_file_read => "not_activation",
            .other_reference => "reference",
        };
    }

    fn activationStateIndex(kind: SkillActivationKind) usize {
        return switch (kind) {
            .explicit_user_call => 0,
            .implicit_assistant_call => 1,
            .injected_skill_block => 2,
            .skill_file_read => 3,
            .other_reference => 4,
        };
    }

    fn resolveSkillActivationWindow(opts: Options) !SkillActivationWindow {
        const until_ms = try parseSkillActivationBoundMillis(opts.until, "--until");
        ...
        return .{
            .since_ms = try parseSkillActivationBoundMillis(opts.since, "--since"),
            .until_ms = until_ms,
        };
    }

    fn parseSkillActivationBoundMillis(raw_opt: ?[]const u8, flag_name: []const u8) !?i64 {
        const raw = raw_opt orelse return null;
        return time_utils.parseIsoTimestampMillis(raw) orelse blk: {
            printCliError("error: skill-audit activation {s} must be an ISO-8601 timestamp with timezone\n", .{flag_name});
            break :blk error.InvalidTimestampArg;
        };
    }

    fn timestampSatisfiesActivationWindow(ts_opt: ?[]const u8, window: SkillActivationWindow) bool {
        const ts = ts_opt orelse return false;
        const ts_ms = time_utils.parseIsoTimestampMillis(ts) orelse return false;
        if (window.since_ms) |since| {
            if (ts_ms < since) return false;
        }
        if (window.until_ms) |until| {
            if (ts_ms > until) return false;
        }
        return true;
    }

    fn activationSnippet(allocator: std.mem.Allocator, text: []const u8, limit: usize) ![]u8 {
        var out: std.ArrayList(u8) = .empty;
        defer out.deinit(allocator);
    ...
    fn containsDollarSkill(text: []const u8, skill: []const u8) bool {
        var pos: usize = 0;
        while (std.mem.indexOfPos(u8, text, pos, "$")) |dollar_pos| {
            const start = dollar_pos + 1;
            const end = start + skill.len;
            if (end <= text.len and eqlIgnoreCaseAscii(text[start..end], skill)) {
                if (end == text.len or !isSkillSignalNameChar(text[end])) return true;
            }
            pos = dollar_pos + 1;
        }
        return false;
    ...
    fn containsBareSkillReference(text: []const u8, skill: []const u8) bool {
        ...
        while (pos + skill.len <= text.len) : (pos += 1) {
            ...
            if (pos > 0 and isSkillSignalNameChar(text[pos - 1])) continue;
            if (end < text.len and isSkillSignalNameChar(text[end])) continue;
            return true;
        }
        return false;
    }

    fn containsSkillPathSegment(text: []const u8, skill: []const u8) bool {
        if (skill.len == 0 or skill.len > text.len) return false;
        var pos: usize = 0;
        while (pos + skill.len <= text.len) : (pos += 1) {
            const end = pos + skill.len;
            if (!eqlIgnoreCaseAscii(text[pos..end], skill)) continue;
            if (pos == 0 or !isPathSep(text[pos - 1])) continue;
            if (end >= text.len or !isPathSep(text[end])) continue;
            return true;
        }
        return false;
    }

    fn containsSkillFileRead(text_opt: ?[]const u8, skill: []const u8) bool {
        const text = text_opt orelse return false;
        return containsIgnoreCaseAscii(text, "SKILL.md") and containsSkillPathSegment(text, skill);
    }

    fn isSkillSignalNameChar(ch: u8) bool {
    ...
    const OwnedTimeBounds = struct {
        since: ?[]u8 = null,
        until: ?[]u8 = null,

        fn deinit(self: OwnedTimeBounds, allocator: std.mem.Allocator) void {
            if (self.since) |value| allocator.free(value);
    ...
    fn appendLastWindowBoundsForField(
        allocator: std.mem.Allocator,
        where_out: *std.ArrayList(spec.WhereClause),
        field: []const u8,
        opts: Options,
    ) !OwnedTimeBounds {
        const raw = opts.last_text orelse return .{};
        const duration_ms = try parseLastWindowMillis(raw);
        const anchor_ms = if (opts.until) |until| blk: {
            break :blk time_utils.parseIsoTimestampMillis(until) orelse {
                printCliError("error: --until must be an ISO-8601 timestamp with timezone when combined with --last\n", .{});
                return error.InvalidTimestampArg;
            };
        } else currentUnixMillis();
        const since_ms = anchor_ms - duration_ms;
        const since = try formatUnixMillisIsoUtcAlloc(allocator, since_ms);
        errdefer allocator.free(since);
        try where_out.append(allocator, .{
            .field = field,
            .op = .gte,
            .value = .{ .scalar = .{ .string = since } },
        });

        var owned = OwnedTimeBounds{ .since = since };
        if (opts.until == null) {
            const until = try formatUnixMillisIsoUtcAlloc(allocator, anchor_ms);
            errdefer allocator.free(until);
            try where_out.append(allocator, .{
                .field = field,
                .op = .lte,
                .value = .{ .scalar = .{ .string = until } },
            });
            owned.until = until;
        }
        return owned;
    ...
    fn formatUnixMillisIsoUtcAlloc(allocator: std.mem.Allocator, ts_ms: i64) ![]u8 {
        const date = time_utils.dateFromTimestampMillis(ts_ms, .utc) orelse return error.InvalidTimestampArg;
        const day_ms = time_utils.daysFromCivil(date.year, date.month, date.day) * 86_400_000;
        const ms_of_day = ts_ms - day_ms;
        const total_seconds = @divFloor(ms_of_day, 1000);
        const millis = @mod(ms_of_day, 1000);
    ...
    fn cmdSkillAudit(allocator: std.mem.Allocator, sessions_root: []const u8, opts: Options) !void {
        const mode = opts.mode orelse "summary";
        if (std.mem.eql(u8, mode, "activation")) {
            return cmdSkillAuditActivation(allocator, sessions_root, opts);
        }
    ...
    fn cmdSkillAuditActivation(allocator: std.mem.Allocator, sessions_root: []const u8, opts: Options) !void {
        const skill = opts.skill orelse return error.MissingSkillArg;
        try validateRolesCsv(opts.roles_csv);
        const window = try resolveSkillActivationWindow(opts);
        const day_filter = deriveSessionDayPathFilterFromWindow(.{
            .since_ms = window.since_ms,
            .until_ms = window.until_ms,
        });
        var paths = try collectJsonlPaths(allocator, sessions_root, day_filter);
        defer freePathList(allocator, &paths);
        const exclude_path = if (opts.exclude_current) try resolveCurrentSessionPathForExclusion(allocator, sessions_root) else null;
        defer if (exclude_path) |path| allocator.free(path);

        var states = [_]SkillActivationState{
            SkillActivationState.init(allocator, .explicit_user_call),
            SkillActivationState.init(allocator, .implicit_assistant_call),
            SkillActivationState.init(allocator, .injected_skill_block),
            SkillActivationState.init(allocator, .skill_file_read),
            SkillActivationState.init(allocator, .other_reference),
        };
        defer for (&states) |*state| state.deinit(allocator);
        ...
        for (paths.items) |path| {
            ...
            try recordSkillActivationMentions(allocator, &states, skill, path, content.?, window, opts);
            try recordSkillActivationReferences(allocator, &states, skill, path, content.?, window, opts);
            try recordSkillActivationToolReads(allocator, &states, skill, path, window);
        }
        ...
        for (&states) |*state| {
            ...
            try row.putOwnedKey("evidence_kind", .{ .string = activationKindName(state.kind) });
            try row.putOwnedKey("rows", .{ .int = @intCast(state.rows) });
            try row.putOwnedKey("sessions", .{ .int = @intCast(state.sessions.count()) });
            try putOptionalString(&row, "first_seen", state.first_seen);
            try putOptionalString(&row, "last_seen", state.last_seen);
            try row.putOwnedKey("example", .{ .string = state.example orelse "" });
            try row.putOwnedKey("activation_verdict", .{ .string = activationVerdict(state.kind) });
            try out_rows.append(allocator, row);
        }

        try output.writeOutput(allocator, opts.format, out_rows.items, skill_audit_activation_columns[0..], opts.out_path);
    }

    fn recordSkillActivationMentions(
        allocator: std.mem.Allocator,
        states: *[5]SkillActivationState,
        skill: []const u8,
        path: []const u8,
        content: []const u8,
    ...
    fn recordSkillActivationReferences(
        ...
        for (messages) |row| {
            ...
            if (!containsBareSkillReference(row.text, skill)) continue;
            if (containsDollarSkill(row.text, skill)) continue;
            try states[activationStateIndex(.other_reference)].record(allocator, row.path, row.timestamp, row.text);
        }
    }

    fn recordSkillActivationToolReads(
        allocator: std.mem.Allocator,
        states: *[5]SkillActivationState,
        skill: []const u8,
        path: []const u8,
        ...
        for (records.items) |record| {
            ...
            try states[activationStateIndex(.skill_file_read)].record(allocator, record.path, record.start_ts, snippet);
        }
    }
    ...
    fn validateRolesCsv(raw_opt: ?[]const u8) !void {
        ...
        if (!any) return error.InvalidModeArg;
    }

    fn csvContainsToken(raw: []const u8, expected: []const u8) bool {
        var split = std.mem.splitScalar(u8, raw, ',');
        while (split.next()) |part_raw| {
            const part = std.mem.trim(u8, part_raw, " \t\r\n");
    ...
    fn appendMessageAuditFilters(
        ...
        const role_values = try appendRolesWhere(allocator, where, opts.roles_csv);
        try owned_lists.keep(allocator, role_values);
        try appendOptionalContains(allocator, where, "text", opts.contains);
        const contains_any_values = try appendCsvContainsAny(allocator, where, "text", opts.contains_any_text);
        try owned_lists.keep(allocator, contains_any_values);
        try appendCsvContainsAll(allocator, where, "text", opts.contains_all_text);
        try appendOptionalRegex(allocator, where, "text", opts.regex);
        return exclude_path;
    }
    ...
    fn cmdMessageAudit(allocator: std.mem.Allocator, sessions_root: []const u8, opts: Options) !void {
        ...
        var where: std.ArrayList(spec.WhereClause) = .empty;
        defer where.deinit(allocator);
        var owned_lists = OwnedScalarLists{};
        defer owned_lists.deinit(allocator);
        const exclude_path = try appendMessageAuditFilters(allocator, sessions_root, &where, opts, &owned_lists);
        defer if (exclude_path) |path| allocator.free(path);
    ...
    fn cmdSkillCohort(allocator: std.mem.Allocator, sessions_root: []const u8, opts: Options) !void {
        ...
        var owned_lists = OwnedScalarLists{};
        defer owned_lists.deinit(allocator);
        const exclude_path = try appendSkillCohortFilters(allocator, sessions_root, &where, opts, true, &owned_lists);
        defer if (exclude_path) |path| allocator.free(path);
    ...
    fn cmdSkillCohortCohortMode(allocator: std.mem.Allocator, sessions_root: []const u8, opts: Options) !void {
        ...
        var owned_lists = OwnedScalarLists{};
        defer owned_lists.deinit(allocator);
        const exclude_path = try appendSkillCohortFilters(allocator, sessions_root, &where, opts, false, &owned_lists);
        defer if (exclude_path) |path| allocator.free(path);
...
fn timestampSatisfiesBounds(ts_opt: ?[]const u8, opts: Options) bool {
    const ts = ts_opt orelse return false;
    if (opts.last_text) |raw_last| {
        const duration_ms = parseLastWindowMillis(raw_last) catch return false;
        const anchor_ms = if (opts.until) |raw_until|
            time_utils.parseIsoTimestampMillis(raw_until) orelse return false
        else
            currentUnixMillis();
        const ts_ms = time_utils.parseIsoTimestampMillis(ts) orelse return false;
        if (ts_ms < anchor_ms - duration_ms) return false;
        if (ts_ms > anchor_ms) return false;
        return true;
    }
...
fn deriveSessionDayPathFilterFromOptions(opts: Options) ?SessionDayPathFilter {
    if (opts.last_text) |raw_last| {
        const duration_ms = parseLastWindowMillis(raw_last) catch return null;
        const anchor_ms = if (opts.until) |raw_until|
            time_utils.parseIsoTimestampMillis(raw_until) orelse return null
        else
            currentUnixMillis();
        return deriveSessionDayPathFilterFromWindow(.{
            .since_ms = anchor_ms - duration_ms,
...
test "resolve-churn-audit bounds tools and counts permits per session" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();

    try tmp.dir.createDirPath(std.Io.Threaded.global_single_threaded.io(), "sessions/2026/05/09");
    const resolved_content =
        "{\"type\":\"session_meta\",\"timestamp\":\"2026-05-09T09:50:00Z\",\"payload\":{\"id\":\"resolve-overlap\",\"cwd\":\"/repo\",\"model\":\"gpt-5\"}}\n" ++
        "{\"type\":\"event_msg\",\"timestamp\":\"2026-05-09T10:00:00Z\",\"payload\":{\"type\":\"user_message\",\"turn_id\":\"t1\",\"message\":\"Use $resolve on this branch.\"}}\n" ++
        "{\"type\":\"response_item\",\"timestamp\":\"2026-05-09T10:00:01Z\",\"payload\":{\"type\":\"function_call\",\"name\":\"apply_patch\",\"call_id\":\"patch-before-fuse\",\"arguments\":\"*** Update File: src/foo.py\\n+new\\n-old\"}}\n" ++
        "{\"type\":\"event_msg\",\"timestamp\":\"2026-05-09T10:00:01.500Z\",\"payload\":{\"type\":\"patch_apply_end\",\"turn_id\":\"t1\",\"call_id\":\"patch-before-fuse\",\"success\":true,\"changes\":{\"files\":1}}}\n" ++
        "{\"type\":\"event_msg\",\"timestamp\":\"2026-05-09T10:00:02Z\",\"payload\":{\"type\":\"agent_message\",\"turn_id\":\"t1\",\"message\":\"Review Governor RGR-V3-MUTATION-PERMIT\\nnormal_form_register: nf-1 status: proposed\\ngovernor_fuse\\nfuse_state: tripped\"}}\n" ++
        "{\"type\":\"event_msg\",\"timestamp\":\"2026-05-09T10:00:02.500Z\",\"payload\":{\"type\":\"agent_message\",\"turn_id\":\"t1\",\"message\":\"RGR-V3-MUTATION-PERMIT\"}}\n" ++
        "{\"type\":\"response_item\",\"timestamp\":\"2026-05-09T10:00:03Z\",\"payload\":{\"type\":\"function_call\",\"name\":\"apply_patch\",\"call_id\":\"patch-after-fuse\",\"arguments\":\"*** Update File: cmd/main.go\\n+main\"}}\n" ++
        "{\"type\":\"event_msg\",\"timestamp\":\"2026-05-09T10:00:03.500Z\",\"payload\":{\"type\":\"patch_apply_end\",\"turn_id\":\"t1\",\"call_id\":\"patch-after-fuse\",\"success\":true,\"changes\":{\"files\":1}}}\n" ++
        "{\"type\":\"event_msg\",\"timestamp\":\"2026-05-09T10:00:04Z\",\"payload\":{\"type\":\"exec_command_end\",\"turn_id\":\"t1\",\"call_id\":\"cas-ok\",\"command\":\"cas review_session --head cmd-head\",\"cwd\":\"/repo\",\"exit_code\":0,\"stdout\":\"{\\\"headSha\\\":\\\"out-head\\\"}\"}}\n" ++
        "{\"type\":\"event_msg\",\"timestamp\":\"2026-05-09T10:00:05Z\",\"payload\":{\"type\":\"exec_command_end\",\"turn_id\":\"t1\",\"call_id\":\"cas-failed\",\"command\":\"cas review_session --head failed-head\",\"cwd\":\"/repo\",\"exit_code\":1,\"stdout\":\"{\\\"headSha\\\":\\\"failed-head\\\"}\"}}\n" ++
        "{\"type\":\"event_msg\",\"timestamp\":\"2026-05-09T10:00:06Z\",\"payload\":{\"type\":\"exec_command_end\",\"turn_id\":\"t1\",\"call_id\":\"rg-review\",\"command\":\"rg review_session\",\"cwd\":\"/repo\",\"exit_code\":0,\"stdout\":\"review_session\"}}\n" ++
        "{\"type\":\"response_item\",\"timestamp\":\"2026-05-09T12:30:00Z\",\"payload\":{\"type\":\"function_call\",\"name\":\"apply_patch\",\"call_id\":\"patch-after-window\",\"arguments\":\"*** Update File: src/late.py\\n+late\"}}\n" ++
        "{\"type\":\"event_msg\",\"timestamp\":\"2026-05-09T12:30:01Z\",\"payload\":{\"type\":\"patch_apply_end\",\"turn_id\":\"t1\",\"call_id\":\"patch-after-window\",\"success\":true,\"changes\":{\"files\":1}}}\n";
    const missing_permit_content =
        "{\"type\":\"session_meta\",\"timestamp\":\"2026-05-09T10:30:00Z\",\"payload\":{\"id\":\"resolve-missing-permit\",\"cwd\":\"/repo\",\"model\":\"gpt-5\"}}\n" ++
        "{\"type\":\"event_msg\",\"timestamp\":\"2026-05-09T10:30:01Z\",\"payload\":{\"type\":\"user_message\",\"turn_id\":\"t2\",\"message\":\"$resolve please\"}}\n" ++
        "{\"type\":\"event_msg\",\"timestamp\":\"2026-05-09T10:30:02Z\",\"payload\":{\"type\":\"agent_message\",\"turn_id\":\"t2\",\"message\":\"Review Governor\\nnormal_form_register: nf-2 status: falsified\"}}\n" ++
        "{\"type\":\"response_item\",\"timestamp\":\"2026-05-09T10:30:03Z\",\"payload\":{\"type\":\"function_call\",\"name\":\"apply_patch\",\"call_id\":\"patch-test\",\"arguments\":\"*** Update File: tests/test_foo.py\\n+test\"}}\n" ++
        "{\"type\":\"event_msg\",\"timestamp\":\"2026-05-09T10:30:04Z\",\"payload\":{\"type\":\"patch_apply_end\",\"turn_id\":\"t2\",\"call_id\":\"patch-test\",\"success\":true,\"changes\":{\"files\":1}}}\n";
    const mention_only_content =
        "{\"type\":\"session_meta\",\"timestamp\":\"2026-05-09T10:40:00Z\",\"payload\":{\"id\":\"resolve-mention\",\"cwd\":\"/repo\",\"model\":\"gpt-5\"}}\n" ++
        "{\"type\":\"event_msg\",\"timestamp\":\"2026-05-09T10:40:01Z\",\"payload\":{\"type\":\"user_message\",\"turn_id\":\"t3\",\"message\":\"I wrote $resolve in prose, but do not run the workflow.\"}}\n";

    try tmp.dir.writeFile(std.Io.Threaded.global_single_threaded.io(), .{ .sub_path = "sessions/2026/05/09/rollout-resolve-overlap.jsonl", .data = resolved_content });
    try tmp.dir.writeFile(std.Io.Threaded.global_single_threaded.io(), .{ .sub_path = "sessions/2026/05/09/rollout-resolve-missing-permit.jsonl", .data = missing_permit_content });
    try tmp.dir.writeFile(std.Io.Threaded.global_single_threaded.io(), .{ .sub_path = "sessions/2026/05/09/rollout-resolve-mention.jsonl", .data = mention_only_content });

    const root_abs = try tmp.dir.realPathFileAlloc(std.Io.Threaded.global_single_threaded.io(), "sessions", std.testing.allocator);
    defer std.testing.allocator.free(root_abs);
    const output_path = try std.fs.path.join(std.testing.allocator, &.{ root_abs, "resolve-churn-windowed.json" });
    defer std.testing.allocator.free(output_path);
    ...
    try std.testing.expect(std.mem.indexOf(u8, got, "\"exclusions\": 1") != null);
    try std.testing.expect(std.mem.indexOf(u8, got, "\"normal_forms\": { \"proposed\": 1, \"falsified\": 1") != null);
    try std.testing.expect(std.mem.indexOf(u8, got, "\"fuse\": { \"required\": 1, \"tripped\": 1, \"mutations_after_trip\": 1, \"production_net_after_trip\": 1 }") != null);
    try std.testing.expect(std.mem.indexOf(u8, got, "\"mutation\": { \"apply_patch_calls\": 3, \"commits\": 0, \"production_insertions\": 2, \"production_deletions\": 1, \"production_net\": 1, \"test_insertions\": 1") != null);
    try std.testing.expect(std.mem.indexOf(u8, got, "\"permits\": { \"required\": 2, \"emitted\": 2, \"missing\": 1 }") != null);
...
test "workflow-audit reports a workflow cohort without cross-session contamination" {
    ...
    {
        const got = try runCommandWithOutput(std.testing.allocator, .workflow_audit, &.{
            "--root",
            root_abs,
            "--workflow",
            "fixed-point-driver",
...
test "query-lift commands run representative dataset wrappers" {
    ...
    const session_content =
        ...
        "{\"type\":\"response_item\",\"timestamp\":\"2026-03-05T10:00:04Z\",\"payload\":{\"type\":\"function_call\",\"name\":\"exec_command\",\"call_id\":\"q2\",\"arguments\":\"{\\\"cmd\\\":\\\"sed -n '1,220p' /Users/tk/.dotfiles/codex/skills/seq/SKILL.md\\\",\\\"workdir\\\":\\\"/tmp/query-lift-repo\\\"}\"}}\n" ++
    ...
    const skill_activation_out = try std.fs.path.join(std.testing.allocator, &.{ root_abs, "skill-audit-activation.json" });
    defer std.testing.allocator.free(skill_activation_out);
    const skill_activation_args = [_][]const u8{
        "--root",
        sessions_abs,
        "--skill",
        "seq",
    ...
    const skill_activation_got = try runCommandWithOutput(std.testing.allocator, .skill_audit, skill_activation_args[0..], skill_activation_out);
    defer std.testing.allocator.free(skill_activation_got);
    try std.testing.expect(std.mem.indexOf(u8, skill_activation_got, "\"evidence_kind\": \"skill_file_read\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, skill_activation_got, "codex/skills/seq/SKILL.md") != null);
    ...
    const show_query_list_out = try std.fs.path.join(std.testing.allocator, &.{ root_abs, "show-query-list.json" });
    defer std.testing.allocator.free(show_query_list_out);
    const show_query_list_args = [_][]const u8{
        "--root",
        sessions_abs,
        "--contains-any",
    ...
    const show_query_list_got = try runCommandWithOutput(std.testing.allocator, .message_audit, show_query_list_args[0..], show_query_list_out);
    ...
    defer std.testing.allocator.free(skill_cohort_last_out);
    const skill_cohort_last_args = [_][]const u8{
        "--root",
        sessions_abs,
        "--skill",
        "seq",
    ...
    const skill_cohort_last_got = try runCommandWithOutput(std.testing.allocator, .skill_cohort, skill_cohort_last_args[0..], skill_cohort_last_out);
    defer std.testing.allocator.free(skill_cohort_last_got);
    try std.testing.expect(std.mem.indexOf(u8, skill_cohort_last_got, "\"cohort_skill\": \"seq\"") != null);
}

test "skill-audit activation classifies evidence and excludes current session" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();

    try tmp.dir.createDirPath(std.Io.Threaded.global_single_threaded.io(), "sessions/2026/03/05");
    const current_session =
        "{\"timestamp\":\"2026-03-05T09:00:00Z\",\"type\":\"session_meta\",\"payload\":{\"id\":\"current-thread\",\"cwd\":\"/tmp/activation\"}}\n" ++
        "{\"type\":\"response_item\",\"timestamp\":\"2026-03-05T09:01:00Z\",\"payload\":{\"type\":\"message\",\"role\":\"user\",\"content\":[{\"type\":\"input_text\",\"text\":\"$universalist current needle\"}]}}\n";
    const other_session =
        "{\"timestamp\":\"2026-03-05T10:00:00Z\",\"type\":\"session_meta\",\"payload\":{\"id\":\"other-thread\",\"cwd\":\"/tmp/activation\"}}\n" ++
        "{\"type\":\"response_item\",\"timestamp\":\"2026-03-05T10:01:00Z\",\"payload\":{\"type\":\"message\",\"role\":\"user\",\"content\":[{\"type\":\"input_text\",\"text\":\"<skill><name>universalist</name></skill>\"}]}}\n" ++
        "{\"type\":\"response_item\",\"timestamp\":\"2026-03-05T10:02:00Z\",\"payload\":{\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"output_text\",\"text\":\"filler\"}]}}\n" ++
        "{\"type\":\"response_item\",\"timestamp\":\"2026-03-05T10:03:00Z\",\"payload\":{\"type\":\"message\",\"role\":\"user\",\"content\":[{\"type\":\"input_text\",\"text\":\"$universalist explicit needle\"}]}}\n" ++
        "{\"type\":\"response_item\",\"timestamp\":\"2026-03-05T10:04:00Z\",\"payload\":{\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"output_text\",\"text\":\"$universalist implicit needle\"}]}}\n" ++
        "{\"type\":\"response_item\",\"timestamp\":\"2026-03-05T10:05:00Z\",\"payload\":{\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"output_text\",\"text\":\"universalist reference needle\"}]}}\n";
    try tmp.dir.writeFile(std.Io.Threaded.global_single_threaded.io(), .{
        .sub_path = "sessions/2026/03/05/rollout-2026-03-05T09-00-00-current-thread.jsonl",
        .data = current_session,
    });
    try tmp.dir.writeFile(std.Io.Threaded.global_single_threaded.io(), .{
        .sub_path = "sessions/2026/03/05/rollout-2026-03-05T10-00-00-other-thread.jsonl",
        .data = other_session,
    });
    ...
    const full_out = try std.fs.path.join(std.testing.allocator, &.{ root_abs, "activation-full.json" });
    defer std.testing.allocator.free(full_out);
    const full_args = [_][]const u8{
        "--root",
        sessions_abs,
        "--skill",
        "universalist",
    ...
    const full_got = try runCommandWithOutput(std.testing.allocator, .skill_audit, full_args[0..], full_out);
    defer std.testing.allocator.free(full_got);
    try std.testing.expect(std.mem.indexOf(u8, full_got, "\"evidence_kind\": \"explicit_user_call\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, full_got, "\"evidence_kind\": \"implicit_assistant_call\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, full_got, "\"evidence_kind\": \"injected_skill_block\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, full_got, "\"evidence_kind\": \"other_reference\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, full_got, "\"rows\": 2") != null);
    try std.testing.expect(std.mem.indexOf(u8, full_got, "\"activation_verdict\": \"not_activation\"") != null);

    test_codex_thread_id = "current-thread";
    defer test_codex_thread_id = null;

    const excluded_out = try std.fs.path.join(std.testing.allocator, &.{ root_abs, "activation-excluded.json" });
    defer std.testing.allocator.free(excluded_out);
    const excluded_args = [_][]const u8{
        "--root",
        sessions_abs,
        "--skill",
        "universalist",
    ...
    const excluded_got = try runCommandWithOutput(std.testing.allocator, .skill_audit, excluded_args[0..], excluded_out);
    defer std.testing.allocator.free(excluded_got);
    try std.testing.expect(std.mem.indexOf(u8, excluded_got, "\"rows\": 2") == null);
    try std.testing.expect(std.mem.indexOf(u8, excluded_got, "current needle") == null);
    try std.testing.expect(std.mem.indexOf(u8, excluded_got, "\"rows\": 1") != null);

    const message_out = try std.fs.path.join(std.testing.allocator, &.{ root_abs, "message-excluded.json" });
    defer std.testing.allocator.free(message_out);
    const message_args = [_][]const u8{
        "--root",
        sessions_abs,
        "--contains",
        "needle",
    ...
    const message_got = try runCommandWithOutput(std.testing.allocator, .message_search, message_args[0..], message_out);
    defer std.testing.allocator.free(message_got);
    try std.testing.expect(std.mem.indexOf(u8, message_got, "current needle") == null);
    try std.testing.expect(std.mem.indexOf(u8, message_got, "explicit needle") != null);

    const workflow_out = try std.fs.path.join(std.testing.allocator, &.{ root_abs, "workflow-excluded.json" });
    defer std.testing.allocator.free(workflow_out);
    const workflow_args = [_][]const u8{
        "--root",
        sessions_abs,
        "--workflow",
        "universalist",
    ...
    const workflow_got = try runCommandWithOutput(std.testing.allocator, .workflow_audit, workflow_args[0..], workflow_out);
    defer std.testing.allocator.free(workflow_got);
    try std.testing.expect(std.mem.indexOf(u8, workflow_got, "current needle") == null);
    try std.testing.expect(std.mem.indexOf(u8, workflow_got, "implicit needle") != null);

    const zero_out = try std.fs.path.join(std.testing.allocator, &.{ root_abs, "activation-zero.json" });
    defer std.testing.allocator.free(zero_out);
    const zero_args = [_][]const u8{
        "--root",
        sessions_abs,
        "--skill",
        "missing-skill",
    ...
    const zero_got = try runCommandWithOutput(std.testing.allocator, .skill_audit, zero_args[0..], zero_out);
    defer std.testing.allocator.free(zero_got);
    try std.testing.expectEqual(@as(usize, 5), countOccurrences(zero_got, "\"rows\": 0"));
}
