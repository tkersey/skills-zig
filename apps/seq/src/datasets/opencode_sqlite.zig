...
pub const Source = enum {
    ...
    pub fn parse(text: []const u8) !Source {
        ...
        if (std.ascii.eqlIgnoreCase(text, "db")) return .db;
        if (std.ascii.eqlIgnoreCase(text, "jsonl")) return .jsonl;
        return error.InvalidOpencodeSource;
    }
};
...
pub const Db = struct {
    ...
    pub fn open(allocator: std.mem.Allocator, db_path: []const u8) !Db {
        ...
        if (c.sqlite3_open(path_z, &raw) != c.SQLITE_OK or raw == null) {
            return error.OpencodeDbOpenFailed;
        }
        return .{ .handle = raw.? };
    }
    ...
    pub fn prepare(self: *Db, allocator: std.mem.Allocator, sql: []const u8) !Stmt {
        ...
        if (rc != c.SQLITE_OK or stmt_ptr == null) return error.OpencodeDbPrepareFailed;
    ...
    pub fn exec(self: *Db, allocator: std.mem.Allocator, sql: []const u8) !void {
        ...
        if (c.sqlite3_exec(self.handle, sql_z, null, null, null) != c.SQLITE_OK) {
            return error.OpencodeDbStepFailed;
        }
    }
};
...
pub const Stmt = struct {
    ...
    pub fn reset(self: *Stmt) !void {
        ...
        if (rc_reset != c.SQLITE_OK) return error.OpencodeDbStepFailed;
        const rc_clear = c.sqlite3_clear_bindings(self.handle);
        if (rc_clear != c.SQLITE_OK) return error.OpencodeDbBindFailed;
    }
    ...
    pub fn step(self: *Stmt) !StepResult {
        ...
        return switch (rc) {
            c.SQLITE_ROW => .row,
            c.SQLITE_DONE => .done,
            else => error.OpencodeDbStepFailed,
        };
    }
...
pub fn epochMsToIso8601Alloc(allocator: std.mem.Allocator, value: ?i64) !?[]u8 {
    const epoch_ms = value orelse return null;
    if (epoch_ms < 0) return null;

    const secs_i64: i64 = @divTrunc(epoch_ms, 1000);
    const millis_i64: i64 = @mod(epoch_ms, 1000);

    const epoch_seconds = std.time.epoch.EpochSeconds{ .secs = @intCast(secs_i64) };
    ...
    return try std.fmt.allocPrint(
        ...
        .{
            ...
            @as(u16, @intCast(millis_i64)),
        },
    );
}

test "epochMsToIso8601Alloc formats UTC timestamps" {
    const text = (try epochMsToIso8601Alloc(std.testing.allocator, 1772550556416)).?;
    defer std.testing.allocator.free(text);
    try std.testing.expectEqualStrings("2026-03-03T15:09:16.416Z", text);
}
