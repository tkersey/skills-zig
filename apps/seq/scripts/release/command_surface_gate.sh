...
if [[ "$hctp_product_available" != true ]]; then
  expected="$(printf '%s\n' "$expected" | rg -v '^(hctp-source|hylo-extract)$')"
fi
...
if [[ "$hctp_product_available" == true ]]; then
  ...
  for feature in \
    ...
    hylo_extract_v1
  do
    if ! printf '%s\n' "$capabilities_json" | rg -q -- "\"${feature}\": true"; then
  ...
  for flag in \
    ...
    '--signing-seed-fd N'
  do
    if ! printf '%s\n' "$hctp_source_help" | rg -q -- "$flag"; then
      echo "hctp-source help missing protected compile/materialize flag: ${flag}" >&2
      exit 1
    fi
  done
  if ! printf '%s\n' "$hctp_source_help" | rg -q -- 'hylo-source-selection-opening/v1'; then
  ...
  for flag in \
    ...
    '--seal-key-output-fd N'
  do
    if ! printf '%s\n' "$hylo_extract_help" | rg -q -- "$flag"; then
  ...
  for feature in \
    ...
    hylo_extract_v1
  do
    if printf '%s\n' "$capabilities_json" | rg -q -- "\"${feature}\""; then
