...
fn runSet(
    ...
    defer goal_core.freeTarget(allocator, target);
    if (opts.dry_run) return emitTarget(
        allocator,
        opts,
        target,
        if (target.would_create_thread) "thread/start + thread/goal/set" else "thread/goal/set",
    );
...
fn runStatus(
    ...
    const before = try requestGoalGet(allocator, client, target.thread_id.?);
    defer goal_core.freeGoalEnvelope(allocator, before);
    if (std.mem.eql(u8, before.goal_json, "null")) try fail(
        allocator,
        opts,
        "goalNotFound",
        "status requires an existing goal",
        3,
    );
    try requestGoalSet(allocator, client, target.thread_id.?, opts);
    const goal = try requestGoalGet(allocator, client, target.thread_id.?);
...
fn resolveTarget(
    ...
    if (opts.latest) {
        ...
        if (allow_create)
            return .{ .thread_id = null, .selected_by = "create", .would_create_thread = true };
...
fn waitForGoal(
    ...
    std.debug.assert(opts.poll_ms > 0);
    // Include the initial request and the final request after crossing the deadline.
    const maximum_polls = @as(u64, opts.timeout_ms) / opts.poll_ms + 2;
    for (0..maximum_polls) |_| {
        const goal = try requestGoalGet(allocator, client, target.thread_id.?);
        defer goal_core.freeGoalEnvelope(allocator, goal);
        ...
        if (millisNow() - started_ms >= opts.timeout_ms) try fail(
            allocator,
            opts,
            "timeout",
            "wait timed out before terminal goal status",
            124,
        );
        try std.Io.sleep(
            std.Io.Threaded.global_single_threaded.io(),
            .fromMilliseconds(opts.poll_ms),
            .awake,
        );
    }
    try fail(allocator, opts, "timeout", "wait timed out before terminal goal status", 124);
}
...
fn emitTarget(
    ...
    const thread_id_json = if (target.thread_id) |id| try goal_core.jsonStringAlloc(
        allocator,
        id,
    ...
    const method_json = if (request_method) |method| try goal_core.jsonStringAlloc(
        allocator,
        method,
    ) else try allocator.dupe(u8, "null");
    ...
    const output = try std.fmt.allocPrint(
        ...
        .{
            ...
            thread_id_json,
            selected_by_json,
            target.created_thread,
            target.would_create_thread,
            method_json,
        },
    );
...
fn emitGoal(
    ...
    const status_json = if (goal.status) |status| try goal_core.jsonStringAlloc(
        allocator,
        status,
    ) else try allocator.dupe(u8, "null");
...
fn fail(
    ...
    if (opts.json) {
        ...
        const detail_json = if (detail) |raw| try goal_core.jsonStringAlloc(
            allocator,
            raw,
        ) else try allocator.dupe(u8, "null");
        ...
        if (detail) |raw| try stderr_writer.interface.print(
            "cas_goal {s}: {s}: {s}\n",
            .{ opts.command.text(), code, raw },
        ) else try stderr_writer.interface.print(
            "cas_goal {s}: {s}\n",
