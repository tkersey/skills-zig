...
- `cas app-server schema` resolves the exact Codex executable, generates or reuses stable and experimental schema bundles, and checks the compact Codex 0.146 contract without running live probes. `cas app-server preflight` adds profile-scoped live probes and fails closed when a required behavior is unavailable. The full profile proves the initialize lifecycle, explicit server-request coverage, bounded overload retry, pinning, paginated and ephemeral forks, executor-root skill/resource reads, selected environment capability-root acceptance, external import history, structured review dispatch, and paginated inquiry transport. Compile-time capability flags report CAS implementation only; runtime support comes from this exact-binary preflight.
...
- The review kernel is a clean schema-4 cutover. Finish or abandon every live attempt created below Codex 0.146 before upgrading. CAS 0.4 rejects an incompatible runtime before `review/start` and does not bridge retired session or tuple-lock state.
...
- CAS sets `excludeTurns:true` on metadata-only `thread/resume` requests under the required Codex 0.146 contract.
...
- `cas capabilities --json` includes compiled feature flags for the current implementation, including `cas_app_server_contract_0146_v1` and `cas_app_server_schema_probe_v1`. It advertises only the current Codex 0.146 compatibility generation.
