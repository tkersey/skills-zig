...
- `cas app-server schema` resolves the exact Codex executable, generates or reuses stable and experimental schema bundles, and checks the version-neutral app-server capability contract without running live probes. `cas app-server preflight` adds profile-scoped live probes and fails closed when a required behavior is unavailable. Each profile requires only the methods and shapes it uses; the full profile additionally audits the complete schema snapshot. The review probe exercises the production request envelopes through dispatch, terminal interruption, and exact turn readback. Codex version and release-channel strings are diagnostic only; compatibility comes from the selected structural and behavioral profile for the exact resolved binary.
- App-server preflight supports stdio, CAS-managed loopback WebSocket, explicit loopback WebSocket, and Unix-domain WebSocket transports. Explicit selections never fall back. `auto` may fall back from managed WebSocket to stdio only when managed startup fails before the first RPC. The report records the selected transport and endpoint identity. An outbound `--code-mode-host` remains distinct from the inbound CAS transport, accepts the released HTTP(S) root-endpoint form, and is reported only as a redacted origin plus digest.
- `cas app-server session` delegates byte-for-byte stdio or listener operation to the selected Codex app-server (`--codex-path` optionally selects the binary). This keeps raw NDJSON responses, notifications, server requests, authenticated WebSocket listener flags, and later additive app-server methods intact. `cas app-server daemon` likewise delegates the released daemon lifecycle commands and exit status. Neither route imposes a Codex version pin; CAS 0.6.5 targets Codex 0.157.0 in the checksum-pinned Linux and macOS qualification lanes. A target version is not itself evidence that qualification passed.
...
- `cas_conformance_suite` verifies bounded overload retry behavior and selected exact-runtime app-server feature probes.
...
- Owner-lived review polls Codex paginated history through `thread/turns/list`; `thread/read includeTurns` remains legacy-only and cannot terminate a current review owner.
- Non-blocking structured findings remain findings when Codex reports `patch is correct`; they are neither rejected as malformed nor credited as clean.
...
- Thread-backed DCPs use `thread_fork` lineage with app-server `thread/fork` and history-mode-specific anchoring: exact completed `lastTurnId` for paginated history, rollback only for a legacy runtime that actually exposes it. Codex 0.157.0 does not expose `thread/rollback`, so the full capability contract no longer requires that retired endpoint. Rollout-backed DCPs with `source.thread_id = null` use `rollout_transcript` lineage: CAS verifies the DCP source and retained-anchor digests from `source.rollout_path`, requires `workspace_policy = transcript_only`, starts a fresh inquiry thread, and sends one bounded transcript-context `turn/start`. Rollout transcript replay is not live workspace reconstruction.
...
- `cas capabilities --json` includes compiled feature flags for the current implementation, including `cas_app_server_contract_v2`, `cas_app_server_schema_probe_v1`, and `cas_structured_review_v1`. These describe CAS capabilities, not a required Codex version.
...
./zig-out/bin/cas conformance --cwd /path/to/workspace --scenario overload_backoff --json
...
The PR CI lanes qualify the exact released Codex 0.157.0 packages on Linux
x86_64 and macOS arm64, verifying archive hashes before executing either binary.
The retained evidence binds the package, executable, and generated schemas.
Successful schema generation alone is not a passing behavioral qualification.
The packaged `codex-code-mode-host` must be beside `codex` for the integration gate.
