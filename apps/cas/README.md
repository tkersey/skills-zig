...
- `cas capabilities --json` includes compiled feature flags for `session_inquiry_v1`, `dcp_v1`, `rip_v1`, `fir_v1`, exact fork/rollback anchoring, ephemeral forks, read-only inquiry, detached inquiry, `cas_rer_opaque_request_binding_v1`, `cas_review_scoped_instructions_v1`, and `cas_codex_0145_structured_review_v3` (`v1` and `v2` remain advertised for compatibility).
