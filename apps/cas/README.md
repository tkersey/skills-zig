...
- `cas review_session receipt proof --clean-streak N` computes a distinct-attempt clean streak from normalized receipts. Cached repeats of the same `reviewThreadId` do not increment the streak; pre-review/no-attempt transport failures are ignored. Strong proof requires receipt metadata bound to a stable redacted account principal. Attempts with missing principal metadata or type/plan-only account evidence remain reduced (`principalProofUsable=false`) and do not certify proof unless `--allow-reduced-principal REASON` is explicit.
...
# Prove a clean streak from distinct tuple-bound review attempts.
...
# Auditable compatibility escape hatch for old/reduced-principal receipts. This is degraded proof,
# not equivalent to strong account-principal certification.
./zig-out/bin/cas review_session receipt proof \
  --glob "$HOME/.codex/cas/review_sessions/*.json" \
  --cwd /path/to/workspace \
  --base main \
