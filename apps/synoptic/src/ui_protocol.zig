...
pub const allowed_commands = [_][]const u8{ "file.open", "session.message", "session.interrupt", "session.close", "approval.resolve", "action.confirm", "action.reject", "snapshot.get", "pr.refresh", "round.finish", "app.stop" };
pub const autonomous_events = [_][]const u8{ "session.item.delta", "approval.requested", "approval.resolved", "action.prepared", "action.superseded", "action.status", "file.completed", "session.closed", "app.stopped" };
pub fn commandAllowed(value: []const u8) bool {
    for (allowed_commands) |candidate| if (std.mem.eql(u8, value, candidate)) return true;
    return false;
}

pub fn visibleEventPayloadAlloc(allocator: std.mem.Allocator, session_id: ?[]const u8, method: []const u8, raw: []const u8) ![]u8 {
    return std.fmt.allocPrint(allocator, "{{\"sessionId\":{f},\"method\":{f},\"raw\":{f}}}", .{ std.json.fmt(session_id orelse "", .{}), std.json.fmt(method, .{}), std.json.fmt(raw, .{}) });
}
