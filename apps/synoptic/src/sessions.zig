...
pub const Registry = struct {
    ...
    pub fn openFile(self: *Registry, cwd: []const u8, path: []const u8, revision: []const u8, base_oid: []const u8, head_oid: []const u8, diff: []const u8, threads_json: []const u8, skill_path: []const u8) !bool {
        ...
        try self.sessions.append(self.allocator, .{ .id = session_id, .thread_id = try self.allocator.dupe(u8, self.file_thread_id.?), .path = try self.allocator.dupe(u8, path), .revision = try self.allocator.dupe(u8, revision) });
        return false;
    }
    ...
    pub fn message(self: *Registry, text: []const u8, active: bool) !void {
        ...
        const params = try std.fmt.allocPrint(self.allocator, "{{\"threadId\":{f},\"expectedTurnId\":{f},\"input\":[{{\"type\":\"text\",\"text\":{f}}}]}}", .{ std.json.fmt(self.file_thread_id orelse return error.NoFileSession, .{}), std.json.fmt(self.file_turn_id orelse "", .{}), std.json.fmt(text, .{}) });
        defer self.allocator.free(params);
    ...
    pub fn interrupt(self: *Registry) !void {
        const actor = &(self.actor orelse return error.AppServerUnavailable);
        const params = try std.fmt.allocPrint(self.allocator, "{{\"threadId\":{f},\"turnId\":{f}}}", .{ std.json.fmt(self.file_thread_id orelse return error.NoFileSession, .{}), std.json.fmt(self.file_turn_id orelse return error.NoActiveTurn, .{}) });
        defer self.allocator.free(params);
    ...
    fn onServerRequest(context: *anyopaque, request: cas_runtime.ServerRequest, allocator: std.mem.Allocator) ![]u8 {
        ...
        if (std.mem.eql(u8, request.method, "item/tool/call")) {
            ...
            if (kind) |event_kind| {
                self.mutex.lock();
                defer self.mutex.unlock();
                for (self.sessions.items) |*session| if (session.status == .current and !session.initial_turn_active and session.human_action_authorized) {
...
test "session authority is immediately governing and close is local" {
    ...
    defer registry.deinit();
    try registry.sessions.append(std.testing.allocator, .{ .id = try std.testing.allocator.dupe(u8, "s"), .thread_id = try std.testing.allocator.dupe(u8, "t"), .path = try std.testing.allocator.dupe(u8, "a"), .revision = try std.testing.allocator.dupe(u8, "r") });
