...
fn launch(allocator: std.mem.Allocator, io: std.Io, environment: *const std.process.Environ.Map, args: []const []const u8) !void {
    ...
    const selector_url = try resolveSelectorUrl(allocator, io, gh_resolved, options.cwd, options.pr); defer allocator.free(selector_url);
    ...
    var file_pages = try broker.callPages(graphql.snapshot_query, "files", identity.owner, identity.repository, identity.number); defer { for (file_pages.items) |p| allocator.free(p); file_pages.deinit(allocator); }
    var thread_pages = try broker.callPages(graphql.threads_query, "reviewThreads", identity.owner, identity.repository, identity.number); defer { for (thread_pages.items) |p| allocator.free(p); thread_pages.deinit(allocator); }
    const snapshot_head = try snapshotField(allocator, file_pages.items[0], "headRefOid"); defer allocator.free(snapshot_head);
    const snapshot_base = try snapshotField(allocator, file_pages.items[0], "baseRefOid"); defer allocator.free(snapshot_base);
    const pull_request_id = try snapshotField(allocator, file_pages.items[0], "id"); defer allocator.free(pull_request_id);
    var generation = try domain.PrGeneration.initFull(allocator, snapshot_base, snapshot_head); errdefer generation.deinit();
    ...
    const managed_path = try std.fmt.allocPrint(allocator, "/tmp/synoptic/{d}/worktree", .{std.c.getpid()}); defer allocator.free(managed_path);
    var custody = try worktree.select(allocator, io, options.cwd, snapshot_head, managed_path); defer allocator.free(custody.path());
    const review_cwd = custody.path();
    try hydrateRevisionKeys(allocator, io, review_cwd, snapshot_base, &generation);
    var state = try App.init(allocator, snapshot_head); state.replaceGeneration(generation); defer state.deinit();

    var registry = try sessions.Registry.start(allocator, review_cwd, codex_resolved); defer registry.deinit();
    try registry.createPrimary(review_cwd); state.primary_ready = registry.primaryReady();
    ...
    if (options.json) try out.interface.print("{{\"schema\":\"synoptic-launch-ready/v1\",\"status\":\"ready\",\"pid\":{d},\"url\":{f},\"repository\":{f},\"pullRequest\":{d},\"worktree\":{f},\"worktreeKind\":{f},\"transport\":\"stdio\"}}\n", .{ std.c.getpid(), std.json.fmt(url, .{}), std.json.fmt(repository, .{}), identity.number, std.json.fmt(review_cwd, .{}), std.json.fmt(custody.kind(), .{}) })
    else try out.interface.print("{s}\n", .{url});
    try out.interface.flush();
    const skill_path = try std.fs.path.join(allocator, &.{ options.skill_root, "SKILL.md" }); defer allocator.free(skill_path);
    ...
    while (true) { state.primary_ready = registry.primaryReady(); server.serveOne(&runtime) catch |err| switch (err) { error.EndOfStream => continue, else => return err }; }
}
...
fn loadSnapshotFiles(allocator: std.mem.Allocator, raw: []const u8, generation: *domain.PrGeneration) !void {
    ...
    for (nodes) |node| { const o = node.object; const state = o.get("viewerViewedState").?.string; const path = o.get("path").?.string; const change_type = o.get("changeType").?.string; const revision = try domain.revisionKey(allocator, path, change_type, "graphql-blob-unavailable", path); defer allocator.free(revision); try generation.addFile(.{ .path = path, .additions = @intCast(o.get("additions").?.integer), .deletions = @intCast(o.get("deletions").?.integer), .change_type = change_type, .viewed = if (std.mem.eql(u8, state, "VIEWED")) .viewed else if (std.mem.eql(u8, state, "DISMISSED")) .dismissed else .unviewed, .revision_key = revision }); }
}
fn loadThreads(allocator: std.mem.Allocator, raw: []const u8, generation: *domain.PrGeneration) !void { var parsed = try std.json.parseFromSlice(std.json.Value, allocator, raw, .{}); defer parsed.deinit(); const pull = (((parsed.value.object.get("data").?).object.get("repository").?).object.get("pullRequest").?).object; for (pull.get("reviewThreads").?.object.get("nodes").?.array.items) |node| { const o = node.object; if (o.get("isResolved").?.bool) continue; const line: ?u32 = if (o.get("line").? == .null) null else @intCast(o.get("line").?.integer); try generation.addThread(.{ .id = o.get("id").?.string, .path = o.get("path").?.string, .line = line, .outdated = o.get("isOutdated").?.bool }); } }
...
fn resolveSelectorUrl(allocator: std.mem.Allocator, io: std.Io, gh_path: []const u8, cwd: []const u8, selector: ?[]const u8) ![]u8 {
    ...
    const result = try std.process.run(allocator, io, .{ .argv = argv, .cwd = .{ .path = cwd } }); defer allocator.free(result.stderr); defer allocator.free(result.stdout);
...
fn hydrateRevisionKeys(allocator: std.mem.Allocator, io: std.Io, cwd: []const u8, base_oid: []const u8, generation: *domain.PrGeneration) !void {
    const fetch = try std.process.run(allocator, io, .{ .argv = &.{ "git", "fetch", "--no-tags", "origin", base_oid }, .cwd = .{ .path = cwd } }); defer allocator.free(fetch.stdout); defer allocator.free(fetch.stderr);
    if (fetch.term != .exited or fetch.term.exited != 0) return error.BaseFetchFailed;
    for (generation.files.items) |file| {
        const spec = try std.fmt.allocPrint(allocator, "HEAD:{s}", .{file.path}); defer allocator.free(spec);
        const blob_result = try std.process.run(allocator, io, .{ .argv = &.{ "git", "rev-parse", "--verify", spec }, .cwd = .{ .path = cwd } }); defer allocator.free(blob_result.stdout); defer allocator.free(blob_result.stderr);
        const blob = if (blob_result.term == .exited and blob_result.term.exited == 0) std.mem.trim(u8, blob_result.stdout, "\r\n") else "DELETION";
        const range = try std.fmt.allocPrint(allocator, "{s}..HEAD", .{base_oid}); defer allocator.free(range);
        const diff_result = try std.process.run(allocator, io, .{ .argv = &.{ "git", "diff", "--no-ext-diff", "--no-color", range, "--", file.path }, .cwd = .{ .path = cwd } }); defer allocator.free(diff_result.stdout); defer allocator.free(diff_result.stderr);
        if (diff_result.term != .exited or diff_result.term.exited != 0) return error.FileDiffFailed;
        const revision = try domain.revisionKey(allocator, file.path, file.change_type, blob, diff_result.stdout); defer allocator.free(revision);
