...
`workspace audit`, `doctor`, and `recover` emit JSON receipts for registry integrity and prepared transaction recovery state. `workspace recover` is inspect-only at the CLI boundary: it reports pending legacy and DTX-v1 transaction journals and returns exit code `2` when manual or library recovery is required. The reusable recovery decision logic lives in `durable_store.inspectTransaction` and `durable_store.recoverTransaction`, which finish already-published commits, abort unpublished prepared transactions, and fail closed on digest disagreement. `workspace export` writes an STW-EXPORT-v1 bundle containing the latest workspace checkpoint plus registered plan JSONL bytes; `workspace import` restores that bundle and refuses to replace an existing workspace unless `--replace` is explicit.

Concurrency and recovery failures are semantic conflicts, not usage errors. Stale sequences, digest mismatches, expectation mismatches, busy or stale locks, fencing-token mismatches, corrupt transaction records, and recovery-required states return exit code `2` with machine-readable receipts or diagnostics where the command surface supports structured output. Usage and system failures remain exit code `1`.
